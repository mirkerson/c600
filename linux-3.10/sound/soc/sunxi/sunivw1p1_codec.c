/*
 * sound\soc\sunxi\sunivw1p1_codec.c
...
//	snd_soc_update_bits(codec, DAC_PA_SRC, (0x1<<RHPPAMUTE), (0x0<<RHPPAMUTE));
	...
	*/
	...
	snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<PH_COM_PROTEC),  (0x1<<PH_COM_PROTEC));
	snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x3<<PH_COM_FC),      (0x3<<PH_COM_FC));
	snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x3F<<HP_VOL),        (0x3F<<HP_VOL));   /* set hp volume */
	snd_soc_update_bits(codec, AC_DAC_DPC,     (0x1<<EN_DAC),         (0x1<<EN_DAC));

	snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<HP_POWER_EN),        (0x1<<HP_POWER_EN)); /* power hp */
    
		snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<L_MIXER_MUTE_R_DAC), (0x1<<L_MIXER_MUTE_R_DAC));
        snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<L_MIXER_MUTE_L_DAC), (0x1<<L_MIXER_MUTE_L_DAC));

		snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<R_MIXER_MUTE_R_DAC), (0x1<<R_MIXER_MUTE_R_DAC));
		snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<R_MIXER_MUTE_L_DAC), (0x1<<R_MIXER_MUTE_L_DAC));
        
		snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<PH_L_MUTE),		  (0x1<<PH_L_MUTE));		
		snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<PH_R_MUTE),		  (0x1<<PH_R_MUTE));

        snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<DAC_AG_L_EN),        (0x1<<DAC_AG_L_EN));		
		snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<DAC_AG_R_EN),        (0x1<<DAC_AG_R_EN));
	return 0;
}
...
static const struct snd_kcontrol_new sunxi_codec_controls[] = {
	SOC_SINGLE_TLV("dac digital volume",                             AC_DAC_DPC,     DVOL,                0x3f, 0, NULL),
	SOC_SINGLE_TLV("dac: right chanel en",                           DAC_MIXER_CTRL, DAC_AG_R_EN,         1,    0, NULL),
	SOC_SINGLE_TLV("dac: left chanle en",                            DAC_MIXER_CTRL, DAC_AG_L_EN,         1,    0, NULL),
	SOC_SINGLE_TLV("dac: right analog output mixer en",              DAC_MIXER_CTRL, R_MIXER_EN,          1,    0, NULL),
	SOC_SINGLE_TLV("dac: left analog output mixer en",               DAC_MIXER_CTRL, L_MIXER_EN,          1,    0, NULL),
	SOC_SINGLE_TLV("dac: right mute",                                DAC_MIXER_CTRL, PH_R_MUTE,           1,    0, NULL),
	SOC_SINGLE_TLV("dac: left mute",                                 DAC_MIXER_CTRL, PH_L_MUTE,           1,    0, NULL),
	SOC_SINGLE_TLV("hp right source select: 0-dac, 1-mixer",         DAC_MIXER_CTRL, PH_R_PWR_SLT,        1,    0, NULL),
	SOC_SINGLE_TLV("hp left source select: 0-dac, 1-mixer",          DAC_MIXER_CTRL, PH_L_PWR_SLT,        1,    0, NULL),
	SOC_SINGLE_TLV("dac: right mixer mute: mic",                     DAC_MIXER_CTRL, R_MIXER_MUTE_MIC,    1,    0, NULL),
	SOC_SINGLE_TLV("dac: right mixer mute: linein",                  DAC_MIXER_CTRL, R_MIXER_MUTE_LINEIN, 1,    0, NULL),
	SOC_SINGLE_TLV("dac: right mixer mute: FM",                      DAC_MIXER_CTRL, R_MIXER_MUTE_FM,     1,    0, NULL),
	SOC_SINGLE_TLV("dac: right mixer mute: right dac",               DAC_MIXER_CTRL, R_MIXER_MUTE_R_DAC,  1,    0, NULL),
	SOC_SINGLE_TLV("dac: right mixer mute: left dac",                DAC_MIXER_CTRL, R_MIXER_MUTE_L_DAC,  1,    0, NULL),
	SOC_SINGLE_TLV("head phone power",                               DAC_MIXER_CTRL, HP_POWER_EN,         1,    0, NULL),
	SOC_SINGLE_TLV("dac: left mixer mute: mic",                      DAC_MIXER_CTRL, L_MIXER_MUTE_MIC,    1,    0, NULL),
	SOC_SINGLE_TLV("dac: left mixer mute: linein",                   DAC_MIXER_CTRL, L_MIXER_MUTE_LINEIN, 1,    0, NULL),
	SOC_SINGLE_TLV("dac: left mixer mute: FM",                       DAC_MIXER_CTRL, L_MIXER_MUTE_FM,     1,    0, NULL),
	SOC_SINGLE_TLV("dac: left mixer mute: right dac",                DAC_MIXER_CTRL, L_MIXER_MUTE_R_DAC,  1,    0, NULL),
	SOC_SINGLE_TLV("dac: left mixer mute: left dac",                 DAC_MIXER_CTRL, L_MIXER_MUTE_L_DAC,  1,    0, NULL),
	SOC_SINGLE_TLV("dac: left hpout to right hpout",                 DAC_MIXER_CTRL, L_HP_TO_R_HP_MUTE,   1,    0, NULL),
	SOC_SINGLE_TLV("dac: right hpout to left hpout",                 DAC_MIXER_CTRL, R_HP_TO_L_HP_MUTE,   1,    0, NULL),
	SOC_SINGLE_TLV("head phone volume",                              DAC_MIXER_CTRL, HP_VOL,              0x3f, 0, NULL),
	SOC_SINGLE_TLV("MICIN GAIN control",                             ADC_MIXER_CTRL, MIC_GAIN_CTL,        0x7,  0, NULL),
	SOC_SINGLE_TLV("LINEIN GAIN control",                            ADC_MIXER_CTRL, LINEIN_VOL,          0x7,  0, NULL),
	SOC_SINGLE_TLV("ADC INPUT GAIN control",                         ADC_MIXER_CTRL, ADC_IN_GAIN_CTL,     0x7,  0, NULL),
	SOC_SINGLE_TLV("COS slop time control for Anti-pop",             ADC_MIXER_CTRL, COS_SLOP_TM,         0x3,  0, NULL),
	SOC_SINGLE_TLV("ADC mixer mute for mic",                         ADC_MIXER_CTRL, ADC_MIX_MUTE_MIC,    0x1,  0, NULL),
	SOC_SINGLE_TLV("ADC mixer mute for FML",                         ADC_MIXER_CTRL, ADC_MIX_MUTE_FML,    0x1,  0, NULL),
	SOC_SINGLE_TLV("ADC mixer mute for FMR",                         ADC_MIXER_CTRL, ADC_MIX_MUTE_FMR,    0x1,  0, NULL),
	SOC_SINGLE_TLV("ADC mixer mute for linein",                      ADC_MIXER_CTRL, ADC_MIX_MUTE_LINEIN, 0x1,  0, NULL),
	SOC_SINGLE_TLV("ADC mixer mute for left ouput",                  ADC_MIXER_CTRL, ADC_MIX_MUTE_L,      0x1,  0, NULL),
	SOC_SINGLE_TLV("ADC mixer mute for right ouput",                 ADC_MIXER_CTRL, ADC_MIX_MUTE_R,      0x1,  0, NULL),
	SOC_SINGLE_TLV("ADC PA speed select",                            ADC_MIXER_CTRL, PA_SPEED_SLT,        0x1,  0, NULL),
	SOC_SINGLE_TLV("ADC FM volume",                                  ADC_MIXER_CTRL, FM_TO_MIX_GAIN,      0x7,  0, NULL),
	SOC_SINGLE_TLV("ADC MIC Boost AMP en",                           ADC_MIXER_CTRL, MIC_BST_AMP_EN,      0x1,  0, NULL),
	SOC_SINGLE_TLV("ADC MIC Boost AMP gain control",                 ADC_MIXER_CTRL, MIC_BOST_GAIN,      0x7,  0, NULL),
...
//	clk_prepare_enable(sunxi_internal_codec->codecclk);
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		snd_soc_update_bits(codec, AC_DAC_FIFOC,   (0x1<<FIFO_FLUSH),         (0x1<<FIFO_FLUSH));/* flush tx fifo */
		//snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<L_MIXER_MUTE_R_DAC), (0x1<<L_MIXER_MUTE_R_DAC));
        //snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<L_MIXER_MUTE_L_DAC), (0x1<<L_MIXER_MUTE_L_DAC));
		//snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<HP_POWER_EN),        (0x1<<HP_POWER_EN)); /* power hp */
		//snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<R_MIXER_MUTE_R_DAC), (0x1<<R_MIXER_MUTE_R_DAC));
        //snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<R_MIXER_MUTE_L_DAC), (0x1<<R_MIXER_MUTE_L_DAC));
		snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<PH_L_PWR_SLT),       (0x0<<PH_L_PWR_SLT));
		snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<PH_R_PWR_SLT),       (0x0<<PH_R_PWR_SLT));
		//snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<PH_L_MUTE),          (0x1<<PH_L_MUTE));		
		//snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<PH_R_MUTE),          (0x1<<PH_R_MUTE));		
//		snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<L_MIXER_EN),         (0x0<<L_MIXER_EN));		
//		snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<R_MIXER_EN),         (0x0<<R_MIXER_EN));
		//snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<DAC_AG_L_EN),        (0x1<<DAC_AG_L_EN));		
		//snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<DAC_AG_R_EN),        (0x1<<DAC_AG_R_EN));
	}else {
		snd_soc_update_bits(codec, AC_ADC_FIFOC,   (0x1<<EN_AD),          (0x1<<EN_AD));
		snd_soc_update_bits(codec, AC_ADC_FIFOC,   (0x1<<ADC_FIFO_FLUSH), (0x1<<ADC_FIFO_FLUSH));	
		snd_soc_update_bits(codec, ADC_MIXER_CTRL, (0x1<<ADC_EN),         (0x1<<ADC_EN));
//		snd_soc_update_bits(codec, ADC_MIXER_CTRL,      (0x7<<MIC_GAIN_CTL),     (0x7<<MIC_GAIN_CTL));
//		snd_soc_update_bits(codec, ADC_MIXER_CTRL,      (0x7<<ADC_IN_GAIN_CTL),  (0x7<<ADC_IN_GAIN_CTL));
//		snd_soc_update_bits(codec, ADC_MIXER_CTRL,      (0x3f<<ADC_MIX_MUTE),    (0x20<<ADC_MIX_MUTE));
//		snd_soc_update_bits(codec, ADC_MIXER_CTRL,      (0x1<<MIC_BST_AMP_EN),   (0x1<<MIC_BST_AMP_EN));
	//	snd_soc_update_bits(codec, ADC_MIXER_CTRL,      (0x7<<MIC_BOST_GAIN),    (0x7<<MIC_BOST_GAIN));
	}
	...
	struct snd_soc_dai *codec_dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		snd_soc_update_bits(codec, AC_DAC_FIFOC,   (0x1<<FIFO_FLUSH),		  (0x1<<FIFO_FLUSH));/* flush tx fifo */
		//snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<L_MIXER_MUTE_R_DAC), (0x0<<L_MIXER_MUTE_R_DAC));
        //snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<L_MIXER_MUTE_L_DAC), (0x0<<L_MIXER_MUTE_L_DAC));
		//snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<HP_POWER_EN),		  (0x0<<HP_POWER_EN)); /* power hp */
		//snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<R_MIXER_MUTE_R_DAC), (0x0<<R_MIXER_MUTE_R_DAC));
		//snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<R_MIXER_MUTE_L_DAC), (0x0<<R_MIXER_MUTE_L_DAC));
        
		//snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<PH_L_MUTE),		  (0x0<<PH_L_MUTE));		
		//snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<PH_R_MUTE),		  (0x0<<PH_R_MUTE));		
//		snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<L_MIXER_EN),		  (0x0<<L_MIXER_EN));		
//		snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<R_MIXER_EN),		  (0x0<<R_MIXER_EN));
		//snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<DAC_AG_L_EN),		  (0x0<<DAC_AG_L_EN));		
		//snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<DAC_AG_R_EN),		  (0x0<<DAC_AG_R_EN));

	} else {
		snd_soc_update_bits(codec, AC_ADC_FIFOC, (0x1<<EN_AD), (0x0<<EN_AD));
		snd_soc_update_bits(codec, AC_ADC_FIFOC, (0x1<<ADC_FIFO_FLUSH), (0x1<<ADC_FIFO_FLUSH));	
		snd_soc_update_bits(codec, ADC_MIXER_CTRL, (0x1<<ADC_EN), (0x0<<ADC_EN));
//		snd_soc_update_bits(codec, ADC_MIXER_CTRL, (0x1<<LINEIN_VOL), (0x0<<LINEIN_VOL));
//		snd_soc_update_bits(codec, ADC_MIXER_CTRL, (0x1<<ADC_IN_GAIN_CTL), (0x3<<ADC_IN_GAIN_CTL));
//		snd_soc_update_bits(codec, ADC_MIXER_CTRL, (0x1<<ADC_MIX_MUTE), (0x0<<ADC_MIX_MUTE));

	}
	
//	clk_disable_unprepare(sunxi_internal_codec->codecclk);
}
	...
	for (i = 0; i < ARRAY_SIZE(codec_sr_s); i++) {
		if (codec_sr_s[i].samplerate ==  params_rate(params)) {
			...
			if(codec_sr_s[i].samplerate%1000) { /*22.5792MHz*/
				if (clk_set_rate(sunxi_internal_codec->pllclk, 22579200)) {
					pr_err("[audio-codec]try to set the pll clk rate failed!\n");
				}
			}else {/*24.576MHz*/
	...
	switch (params_format(params)) {
		...
		case SNDRV_PCM_FORMAT_S32_LE:
			if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
				...
				snd_soc_update_bits(codec, AC_DAC_FIFOC, (0x3<<FIFO_MODE), (0x0<<FIFO_MODE));
				snd_soc_update_bits(codec, AC_DAC_FIFOC, (0x1<<TX_SAMPLE_BITS), (0x1<<TX_SAMPLE_BITS));
                snd_soc_update_bits(codec, AC_DAC_FIFOC, (0x1<<AC_ADC_RXDATA), (0x0<<AC_ADC_RXDATA));//added by Alfnoso
			} else {
		...
		default:
			if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
                ...
                snd_soc_update_bits(codec, AC_DAC_FIFOC, (0x1<<AC_ADC_RXDATA), (0x1<<AC_ADC_RXDATA));//added by Alfnoso
			} else {
...
static int codec_soc_remove(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x1<<PH_COM_PROTEC), (0x0<<PH_COM_PROTEC));
	snd_soc_update_bits(codec, DAC_MIXER_CTRL, (0x3<<PH_COM_FC), (0x0<<PH_COM_FC));
		...
		const char *buf, size_t count)
{
	...
	if (input_reg_group == 1) {
		if (rw_flag) {
//			printk("write->reg:0x%08x val:0x%08x\n", codec_digitaladress + input_reg_offset, input_reg_val);
			writel(input_reg_val, codec_digitaladress + input_reg_offset);
//			printk("read ->reg:0x%08x val:0x%08x\n", codec_digitaladress + input_reg_offset, readl(codec_digitaladress + input_reg_offset));
		} else {
...
//	struct gpio_config config;
	...
	if (IS_ERR(sunxi_internal_codec->codecclk)){
		dev_err(&pdev->dev, "[audio-cpudai]Can't get audio codec clock\n");
