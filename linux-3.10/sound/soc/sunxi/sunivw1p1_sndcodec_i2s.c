...
static __mclk_set_inf  MCLK_INF[] =
{
    ...
    //44.1k bitrate
    { 44100, 256,  2, 1},{ 44100, 128,  4, 1}, { 44100, 256,  2, 1}, { 44100, 512,  1, 1},
                                       ...
                                       struct snd_pcm_hw_params *params)
{
	...
	//unsigned long period_size = params_period_size(params);
	//unsigned long buffer_size = params_buffer_size(params);
	//unsigned long format = params_format(params);
	//add wy
	//printk("machine drivers channels:%u period_size:%u buffer_size:%u format: %u\n",channels,period_size,buffer_size,format);
	...
	ret = snd_soc_dai_set_fmt(codec_dai, SND_SOC_DAIFMT_I2S|
			SND_SOC_DAIFMT_NB_NF | SND_SOC_DAIFMT_CBS_CFS);//SND_SOC_DAIFMT_I2S
	if (ret < 0)
		return ret;
...
//	printk("\nwy snd_soc_dai_set_mclk_div\n");
	ret = snd_soc_dai_set_clkdiv(codec_dai, 0, mclk_div);
   if (ret < 0)
		return ret;
//	printk("\nwy snd_soc_dai_set_bclk_div\n");
	ret = snd_soc_dai_set_clkdiv(codec_dai, 1, bclk_div);
   if (ret < 0)
		return ret;

 //  printk("\nwy snd_soc_dai_set_mult_fs\n");
	ret = snd_soc_dai_set_clkdiv(codec_dai, 2, mult_fs);
   if (ret < 0)
		return ret;
...
static int __init sunxi_machine_i2s_init(void)
{
	...
	//printk("\n-----[I2S]sunxi_machine_i2s_init register i2s start------\n");
	if ((err = platform_driver_register(&sunxi_machine_driver)) < 0)
		return err;


	//printk("\n-----[I2S]sunxi_machine_i2s_init register i2s end------\n");
	return 0;
}
