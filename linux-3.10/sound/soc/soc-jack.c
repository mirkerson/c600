...
static int  soc_request_gpio(struct gpio_config *pinConfig)
{
...
/**
 * snd_soc_jack_add_gpio - Associate GPIO pins with an ASoC jack
 *
 * @jack:  ASoC jack
 * @irq: irq num of the pin
 * @gpio: gpio pin
 *
 * This function will request gpio, set data direction and request irq
 * for each gpio in the array.
 */
int snd_soc_jack_add_gpio(struct snd_soc_jack *jack, int irq,
			struct snd_soc_jack_gpio *gpio)
{
	int ret;
	struct gpio_config pin_cfg;
		...
		pin_cfg.gpio=gpio->gpio;
		pin_cfg.drv_level=1;//SUNXI_DRV_LEVEL1;
		pin_cfg.mul_sel=6;//
		pin_cfg.data=1;
		pin_cfg.pull=1;
		soc_request_gpio(&pin_cfg);
		INIT_DELAYED_WORK(&gpio->work, gpio_work);
		gpio->jack = jack;
		ret = request_irq(irq, 
							  (irq_handler_t)gpio_handler,
							   IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "hp_irq", gpio);
		if (ret < 0)
			goto err;
...
err:
	gpio_free(gpio->gpio);
undo:
	snd_soc_jack_free_gpio(jack, irq, gpio);

	return ret;
}
EXPORT_SYMBOL_GPL(snd_soc_jack_add_gpio);

/**
 * snd_soc_jack_free_gpio - Release GPIO pin's resources of an ASoC jack
 *
 * @jack:  ASoC jack
 * @irq: irq num of the pin
 * @gpios: the gpio pin
 ...
 * Release gpio and irq resources for gpio pins associated with an ASoC jack.
 */
void snd_soc_jack_free_gpio(struct snd_soc_jack *jack, int irq,
			struct snd_soc_jack_gpio *gpio)
{
...
EXPORT_SYMBOL_GPL(snd_soc_jack_free_gpio);
