...
build_kernel()
{
	...
	if [ "${LICHEE_CHIP}" = "sun8iw10p1" -o "${LICHEE_CHIP}" = "sunivw1p1" ]; then
    ...
    if [ "x$SUNXI_SPARSE_CHECK" = "x" ] && [ "x$SUNXI_SMATCH_CHECK" = "x" ];then
        ...
        make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} -j${LICHEE_JLEVEL} uImage
    else
...
regen_rootfs_cpio()
{
	...
	cp -a ${LICHEE_MOD_DIR} ./skel/lib/modules
...
rebuild_kernel()
{
    echo "rebuilding kernel for update new ramfs"
