...
static inline void twi_enable_irq(void __iomem *base_addr)
{
	...
	I2C_DBG("%s: set TWI_CTL_REG = 0x%x\n", __func__, reg_val);
	writel(reg_val, base_addr + TWI_CTL_REG);
}
...
static inline void twi_soft_reset(void __iomem *base_addr)
{
	...
	I2C_DBG("%s: set TWI_SRST_REG = 0x%x\n", __func__, reg_val);
...
static inline void twi_set_efr(void __iomem *base_addr, unsigned int efr)
{
	...
	I2C_DBG("%s: set TWI_EFR_REG = 0x%x\n", __func__, reg_val);
...
msg_null:
	...
	I2C_DBG("i2c err_code=0x%x, state=0x%x\n", err_code, state);
	return ret;
}
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:i2c-suniv");
MODULE_DESCRIPTION("SUNIV I2C Bus Driver");
