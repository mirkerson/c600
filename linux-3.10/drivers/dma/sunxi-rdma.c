...
#define NR_MAX_CHAN	4//8			/* total of channels */
#else
...
#ifndef CONFIG_OF
#define RDMA_IRQ_ID		0x12/* SUN9I: 67 */
#define RDMA_PHYS_BASE	0x01c02000	/* SUN9I: 0x08008000 */
#endif

#ifdef	CONFIG_ARCH_SUNIVW1P1
#define RDMA_IRQ_EN		(0x00)					/* Interrupt enable register */
#define RDMA_IRQ_PEN 	(0x04)					/* Inetrrupt status register */
#define	RDMA_PRI_CFG 	(0X08)					/* priority configure register*/
#define RDMA_N_CTRL(x)  (0x100 + ((x) << 5))	/* NDMA control register */	
#define RDMA_N_SRC(x)	(0x104 + ((x) << 5))	/* NDMA source address register */
#define RDMA_N_DST(x)	(0x108 + ((x) << 5))	/* NDMA destination address register */
#define RDMA_N_CNT(x)	(0x10C + ((x) << 5))	/* NDMA byte counter register */
#define RDMA_D_CTRL(x)  (0x300 + ((x) << 5))	/* DDMA Control register */	
#define RDMA_D_SRC(x)	(0x304 + ((x) << 5))	/* NDMA source address register */
#define RDMA_D_DST(x)	(0x308 + ((x) << 5))	/* NDMA destination address register */
#define RDMA_D_CNT(x)	(0x30C + ((x) << 5))	/* NDMA byte counter register */
#define RDAM_D_PAR(x)	(0x318 + ((x) << 5))	/* DDMA parameter register */
#define RDAM_D_GEN(x)	(0x318 + ((x) << 5))	/* DDMA general data register */
#else
#define RDMA_IRQ_EN		(0x00)			/* Interrupt enable register */
#define RDMA_IRQ_PEN	(0x04)			/* Inetrrupt status register */
...
static size_t sunxi_get_chan_size(struct sunxi_chan *ch)
{
	...
	/*read number of left bytes in on channel*/
	ctrl_reg = sunxi_smc_readl(sdev->base + RDMA_N_CTRL(chan_num));
	sunxi_smc_writel(REMAIN_MODE | ctrl_reg, sdev->base + RDMA_N_CTRL(chan_num));
	size = sunxi_smc_readl(sdev->base + RDMA_N_CNT(chan_num));
	ctrl_reg = sunxi_smc_readl(sdev->base + RDMA_N_CTRL(chan_num));
	sunxi_smc_writel(ctrl_reg & (~REMAIN_MODE), sdev->base + RDMA_N_CTRL(chan_num));
...
static inline void sunxi_dump_com_regs(struct sunxi_chan *ch)
{
	...
	pr_debug("Common Register: \n"
			...
			sunxi_smc_readl(sdev->base + RDMA_PRI_CFG));
}
...
static inline void sunxi_dump_chan_regs(struct sunxi_chan *ch)
{
	...
	pr_debug("CHAN: %d, Register:\n"
			...
			chan_num,
			RDMA_N_SRC(chan_num),
			sunxi_smc_readl(sdev->base + RDMA_N_SRC(chan_num)),
			RDMA_N_DST(chan_num),
			sunxi_smc_readl(sdev->base + RDMA_N_DST(chan_num)),
			RDMA_N_CTRL(chan_num),
			sunxi_smc_readl(sdev->base + RDMA_N_CTRL(chan_num)),
			RDMA_N_CTRL(chan_num),
			sunxi_smc_readl(sdev->base + RDMA_N_CTRL(chan_num)));
}
...
static int sunxi_terminate_all(struct sunxi_chan *ch)
{
	...
	ch->cyclic = false;
	sunxi_smc_writel(~CHAN_START, sdev->base + RDMA_N_CTRL(chan_num));
...
static void sunxi_start_desc(struct sunxi_chan *ch)
{
	...
	ch->desc = txd;
	while(sunxi_smc_readl(sdev->base + RDMA_N_CTRL(chan_num)) & BUSY_BIT)
		cpu_relax();
	ch->irq_type = IRQ_PKG;

	if (ch->cyclic){
		ch->irq_type |= IRQ_HALF;
		sunxi_smc_writel(1 << 16, sdev->base + RDMA_PRI_CFG);
	}
	...
	sunxi_smc_writel(txd->cur_lli->src, sdev->base + RDMA_N_SRC(chan_num));
	sunxi_smc_writel(txd->cur_lli->dst, sdev->base + RDMA_N_DST(chan_num));
	sunxi_smc_writel(txd->cur_lli->len, sdev->base + RDMA_N_CNT(chan_num));
	sunxi_smc_writel(txd->cur_lli->cfg | CHAN_START, sdev->base + RDMA_N_CTRL(chan_num));
...
static struct sunxi_rdma_lli *sunxi_lli_load(struct sunxi_chan *chan)
{
	...
	sunxi_smc_writel(l_item->src, sdev->base + RDMA_N_SRC(chan_num));
	sunxi_smc_writel(l_item->dst, sdev->base + RDMA_N_DST(chan_num));
...
#ifdef CONFIG_OF
	pdev->dev.dma_mask = &sunxi_rdma_mask;
...
#ifdef	CONFIG_OF
	sunxi_dev->apbs_clk = clk_get(&pdev->dev, "dma");
	if (!sunxi_dev->apbs_clk) {
...
#ifdef	CONFIG_OF
	clk_prepare_enable(sunxi_dev->apbs_clk);
#else
	sunxi_smc_writel(sunxi_smc_readl((void *)(0xf1c20000 + 0x2c0)) | (1 << 6), (void *)(0xf1c20000 + 0x2c0));
	sunxi_smc_writel(sunxi_smc_readl((void *)(0xf1c20000 + 0x60))  | (1 << 6), (void *)(0xf1c20000 + 0x60));
#endif	
	return 0;
...
static struct resource sunxi_rdma_reousce[] = {
	[0] = {
		...
		.end = RDMA_PHYS_BASE + RDAM_D_GEN(NR_MAX_CHAN),
