/*
 * Copyright (C) 2016 Emilio L¡§?pez
 * Emilio L¡§?pez <zhuangqiubin@allwinnertech.com>
 *
...
#define sun3i_dma_warning  pr_warning
/****************Patch for DMA Bug*********************************
...
/*way4:xxw wait the dma end before terminate*/
//#define TERMINATE_WAIT_DMA
//#define DO_A_INNER_TRANSFER

/*way5:do inner transfer to clean all the pchan*/
//#define TERMINATE_WAIT_DMA
//#define DO_INNER_TRANSFER_IN_ALL
//#define DO_INNER_TRANSFER_IN_ALL2


/** debug help **/
#define DETECT_PCHAN_ADDR
			  ...
			  struct sun3i_dma_pchan *pchan)
{
	...
	spin_lock_irqsave(&priv->lock, flags);
	cfg_reg = readl_relaxed(pchan->base + SUN3I_NDMA_CFG_REG);
	if (cfg_reg & 0x80000000) {
		sun3i_dma_warning("release the pchan %d which is busy\n",nr);
	}
	pchan->vchan = NULL;
...
static int is_addr_covered(void *addr1, u32 len1, void *addr2, u32 len2)
{
	if(((addr1 > addr2) && (addr1 < addr2 + len2))
		||((addr1 + len1 > addr2) && (addr1 + len1 < addr2 + len2))) {
		sun3i_dma_warning("addr converd! %x,%d,%x,%d\n",(u32)addr1,len1,(u32)addr2,len2);
		return 1;
	}
	return 0;
}
...
/*
 * check if other pchan using these address
 */
static int sanitize_pchan_addr(struct sun3i_dma_dev *priv,struct sun3i_dma_pchan *pchan,
				struct sun3i_dma_promise *d)
{
	int nr,i;
	u32 len_src, len_dst, len_src_pro, len_dst_pro;
	struct sun3i_dma_promise *pro;
	struct sun3i_dma_pchan *pchans = priv->pchans;
	struct sun3i_dma_vchan *vchan;
	nr = pchan-priv->pchans;
	len_src = len_dst = d->len;
	/* IO mode */
	if (d->cfg & 0x20)
		len_src = 0;
	if (d->cfg & 0x200000)
	...
	for (i = 0; i < SUN3I_NDMA_NR_MAX_CHANNELS; i++) {
		...
		if (!vchan->processing)
			continue;
		pro = vchan->processing;
		/* someone have processing, check now */
		len_src_pro = len_dst_pro = pro->len;
		if (pro->cfg & 0x20)
			len_src_pro = 0;
		if (pro->cfg & 0x200000)
			len_dst_pro = 0;

		if (is_addr_covered((void *)d->src, len_src, (void *)pro->src, len_src_pro)
				|| is_addr_covered((void *)d->dst, len_dst, (void *)pro->src, len_src_pro)
				|| is_addr_covered((void *)d->src, len_src, (void *)pro->dst, len_dst_pro)
				|| is_addr_covered((void *)d->dst, len_dst, (void *)pro->dst, len_dst_pro)) {
			sun3i_dma_warning("addr cover!!!!!!!!!!!!!!!!\n");
			return -1;
		}
	}
	return 0;
...
static void configure_pchan(struct sun3i_dma_dev *priv,struct sun3i_dma_pchan *pchan,
				struct sun3i_dma_promise *d)
{
#ifdef DO_INNER_TRANSFER_IN_ALL
	/*
	 * if pchan is dirty, do nothing.
	 * the vchan user do not need to know this,
	 * the promise will be configure to pchan after all pchan clean
	 */
	if(pchan->is_dirty)
		return;
#endif

#ifdef DETECT_PCHAN_ADDR
	sanitize_pchan_addr(priv, pchan, d);
#endif
...
static int is_some_NDMA_pchans_dirty(struct sun3i_dma_dev *priv)
{
	int i;
	unsigned long flags;
...
static int is_all_NDMA_pchans_ended(struct sun3i_dma_dev *priv)
{
	int i;
	u32 cfg_reg;
	unsigned long flags;
...
static int is_all_NDMA_pchans_dirty(struct sun3i_dma_dev *priv)
{
	int i;
	unsigned long flags;
	struct sun3i_dma_pchan *pchans = priv->pchans, *pchan;
...
static int is_all_NDMA_pchans_dirty_and_ended(struct sun3i_dma_dev *priv)
{
	if (is_all_NDMA_pchans_dirty(priv)
		&& is_all_NDMA_pchans_ended(priv))
		return 1;
	return 0;
}

static void start_the_inner_transfer(struct sun3i_dma_pchan *pchan)
{
	u32 len = SUN3I_PCHAN_INNER_BUFFER_LEN;
	if (pchan->is_dedicated) {
		writel_relaxed(pchan->inner_dma_src,
				pchan->base +SUN3I_DDMA_SRC_ADDR_REG);
		writel_relaxed(pchan->inner_dma_dst,
				pchan->base +SUN3I_DDMA_DST_ADDR_REG);
		writel_relaxed(len,
				pchan->base +SUN3I_DDMA_BYTE_COUNT_REG);
		writel_relaxed(SUN3I_DDMA_SDRAM2SDRAM_CFG,
				pchan->base +SUN3I_DDMA_CFG_REG);
	} else {
		writel_relaxed(pchan->inner_dma_src,
				pchan->base +SUN3I_NDMA_SRC_ADDR_REG);
		writel_relaxed(pchan->inner_dma_dst,
				pchan->base +SUN3I_NDMA_DST_ADDR_REG);
		writel_relaxed(len,
				pchan->base +SUN3I_NDMA_BYTE_COUNT_REG);
		writel_relaxed(SUN3I_NDMA_SDRAM2SDRAM_CFG,
				pchan->base +SUN3I_NDMA_CFG_REG);
	}
	return ;
}

/* only for NDMA now */
static void start_inner_transfer_for_some_pchans(struct sun3i_dma_dev *priv, int pchans_to_clean)
{
	int i;
	unsigned long flags;
	struct sun3i_dma_pchan *pchans = priv->pchans, *pchan;
	...
	for (i = 0; i < SUN3I_NDMA_NR_MAX_CHANNELS; i++) {
		pchan = &pchans[i];
		if (pchans_to_clean & (1 << i))
			start_the_inner_transfer(pchan);
	}
	spin_unlock_irqrestore(&priv->lock, flags);
	return ;
}

static void start_all_NDMA_inner_transfer(struct sun3i_dma_dev *priv)
{
	int i;
	unsigned long flags;
	struct sun3i_dma_pchan *pchans = priv->pchans, *pchan;
...
static void start_all_dirty_NDMA_inner_transfer(struct sun3i_dma_dev *priv)
{
	int i;
	unsigned long flags;
	struct sun3i_dma_pchan *pchans = priv->pchans, *pchan;
...
static void restore_NDMA_after_clean(struct sun3i_dma_dev *priv)
{
	int i;
	struct sun3i_dma_pchan *pchans = priv->pchans, *pchan;
	struct sun3i_dma_vchan *vchan;
	for (i = 0; i < SUN3I_NDMA_NR_MAX_CHANNELS; i++) {
		...
		if (!vchan->processing)
			continue;
		set_pchan_interrupt(priv, pchan, vchan->contract->is_cyclic, 1);
		configure_pchan(priv, pchan, vchan->processing);
	}
}

static void set_pchan_dirty_flags(struct sun3i_dma_dev *priv,
		struct sun3i_dma_pchan *pchan)
{
	unsigned long flags;
	spin_lock_irqsave(&priv->lock, flags);
...
static void set_all_NDMA_dirty_flags(struct sun3i_dma_dev *priv)
{
	int i;
	unsigned long flags;
	struct sun3i_dma_pchan *pchans = priv->pchans, *pchan;
...
static void set_all_ended_NDMA_dirty_flags(struct sun3i_dma_dev *priv)
{
	int i;
	u32 cfg_reg;
	unsigned long flags;
...
static void clear_all_NDMA_dirty_flags(struct sun3i_dma_dev *priv)
{
	int i;
	unsigned long flags;
	struct sun3i_dma_pchan *pchans = priv->pchans, *pchan;
				   ...
				   struct sun3i_dma_vchan *vchan)
{
	...
	if (promise) {
		...
		configure_pchan(priv,pchan, promise);
	}
...
static int wait_pchan_end(struct sun3i_dma_pchan *pchan, int delay_cfg)
{
	volatile int delay = delay_cfg;
	unsigned long cfg_reg = 0x80000000;
	/* when the dma end, it clear this bit automatically */
	/* TODO:do not use delay--,use the API of delay in kernel */
	while ((cfg_reg & 0x80000000) && delay--) {
		if (pchan->is_dedicated)
			cfg_reg = readl_relaxed(pchan->base +
						SUN3I_DDMA_CFG_REG);
		else
			cfg_reg = readl_relaxed(pchan->base +
						SUN3I_NDMA_CFG_REG);
...
static int sun3i_dma_terminate_all(struct dma_chan *chan)
{
	...
	if (vchan->terminating) {
		sun3i_dma_warning("terminating %d,do not terminaye again\n",chan->chan_id);
		return 0;
	}
#endif
...
#ifdef DO_INNER_TRANSFER_IN_ALL
		if (pchan->is_dirty) {
			sun3i_dma_warning("should not terminate a dirty pchan,%d\n",pchan-priv->pchans);
		}
#endif

		spin_unlock_irqrestore(&vchan->vc.lock, flags);
		delay = 10;
		pchan_busy = wait_pchan_end(pchan, delay);
		if (pchan_busy) {
#ifdef DO_A_INNER_TRANSFER
			if (pchan->is_dedicated) {
				cfg_reg = readl_relaxed(pchan->base + SUN3I_DDMA_CFG_REG);
				//sun3i_dma_warning("cfg_reg = %x\n",cfg_reg);
			...
			start_the_inner_transfer(pchan);
			delay = 10;
			//sun3i_dma_warning("inner...\n");
			pchan_busy = wait_pchan_end(pchan, delay);
			if (pchan_busy){
				sun3i_dma_warning("xxw inner...done, cfg:0x%x, busy=%d\n", cfg_reg, pchan_busy);
			}

#endif

#ifdef DO_INNER_TRANSFER_IN_ALL
			sun3i_dma_warning("termi ,set %d dirty\n",pchan-priv->pchans);
			set_pchan_dirty_flags(priv, pchan);
			sun3i_dma_warning("termi ,after set %d dirty\n",pchan-priv->pchans);
			/* set dirty flags so will not start new transfer */
			set_all_ended_NDMA_dirty_flags(priv);
#ifdef DO_INNER_TRANSFER_IN_ALL2
			start_all_dirty_NDMA_inner_transfer(priv);
#else
			if (is_all_NDMA_pchans_dirty(priv)) {
				sun3i_dma_warning("start all inner in termi\n");
				start_all_NDMA_inner_transfer(priv);
			} else {
				set_pchan_interrupt(priv, pchan, 0, 0);
			}
#endif /* DO_INNER_TRANSFER_IN_ALL2 */
#endif
		}
		spin_lock_irqsave(&vchan->vc.lock, flags);
	}
...
#ifdef DO_INNER_TRANSFER_IN_ALL
	int start_inner_transfer = 0;
	int start_restore = 0;
	int pchans_to_clean = 0;
#endif
...
handle_pending:
	...
	for_each_set_bit(bit, &pendirq, 32) {
		...
		if (pchan->is_dirty) {
			sun3i_dma_warning("int dirty,%d\n",pchan-priv->pchans);
			disableirqs |= BIT(bit);
			/*
			 * this is a inner transfer,
			 * do nothing but clear irq.
			 * when all the dirty transfer end,
			 * clear dirty flags and begin to serve vhans
			 *
			 */
			if (is_all_NDMA_pchans_ended(priv)) {

#ifdef DO_INNER_TRANSFER_IN_ALL2
				if (is_all_NDMA_pchans_dirty(priv) && (pchans_to_clean == 0)) {
					sun3i_dma_warning("int dirty,%d, all dirty and ended, ok to restore\n",pchan-priv->pchans);
					start_restore = 1;
				}
#else
				if (is_all_NDMA_pchans_dirty(priv)) {
					...
					sun3i_dma_warning("int dirty,%d, and all ended, ok to inner transfer\n",pchan-priv->pchans);
					start_inner_transfer = 1;
				}
#endif
			}
			continue;
		} else if (is_some_NDMA_pchans_dirty(priv)) {
			sun3i_dma_warning("int set dirty,%d\n",pchan-priv->pchans);
			disableirqs |= BIT(bit);
			set_pchan_dirty_flags(priv, pchan);
#ifdef  DO_INNER_TRANSFER_IN_ALL2
			pchans_to_clean|=(1 << (bit >> 1));
#else
			if (is_all_NDMA_pchans_ended(priv)) {
				sun3i_dma_warning("int set dirty,%d, and all ended\n",pchan-priv->pchans);
				set_all_NDMA_dirty_flags(priv);
				start_inner_transfer = 1;
			}
#endif
		}
...
#ifdef  DO_INNER_TRANSFER_IN_ALL2
	if (pchans_to_clean) {
		start_inner_transfer_for_some_pchans(priv, pchans_to_clean);
	}
#endif

#ifdef DO_INNER_TRANSFER_IN_ALL
	if (start_inner_transfer == 1) {
		sun3i_dma_warning("start all inner \n");
		set_all_NDMA_dirty_flags(priv);
		start_all_NDMA_inner_transfer(priv);
	}
	else if (start_restore == 1) {
		sun3i_dma_warning("restore NDMA \n");
		...
		if (pendirq)
			writel_relaxed(pendirq, priv->base + SUN3I_DMA_IRQ_PENDING_STATUS_REG);
		clear_all_NDMA_dirty_flags(priv);
		restore_NDMA_after_clean(priv);
	}
#endif
	return IRQ_HANDLED;
}
