...
__s32 mem_enable_int(enum interrupt_source_e src)
{
	...
	*(volatile __u32 *)(int_reg_base + SW_INT_ENABLE_REG0 + tmpGrp * 4) |= (1 << tmpSrc);

	*(volatile __u32 *)(int_reg_base + SW_INT_MASK_REG0 + tmpGrp * 4) &= ~(1 << tmpSrc);
...
//	printk("int_reg_base + SW_INT_PENDING_REG0 + tmpGrp*4 = 0x%x. tmpGrp = 0x%x.\n", int_reg_base + SW_INT_PENDING_REG0 + tmpGrp*4, tmpGrp); 
