...
#ifdef CONFIG_ARCH_SUNIV
static DEFINE_SPINLOCK(clksrc_lock);
#define SUNIV_HRES_CLK_RATE 			24000000
#define SUNIV_CLKSRC_ID				1

static cycle_t suniv_clksrc_read(struct clocksource *cs)
{
	unsigned long flags;
	u32 cnt = 0;

	spin_lock_irqsave(&clksrc_lock, flags);
	cnt = readl(timer_base + TIMER_CNTVAL_REG(SUNIV_CLKSRC_ID));
	cnt = 0xffffffff - cnt;
...
static struct clocksource suniv_clocksrc =
{
	.name = "suniv high-res couter",
	.list = {NULL, NULL},
	.rating = 300,                  /* perfect clock source             */
	.read = suniv_clksrc_read,         /* read clock counter               */
	.enable = 0,                    /* not define                       */
...
u32 suniv_sched_clock_read(void)
{
	u32   cnt = 0;
	unsigned long   flags;
...
void __init suniv_clocksource_init(void)
{
	u32 data = 0;

	//set interval value to max(0xffffffff)
	writel(0xffffffff, timer_base + TIMER_INTVAL_REG(SUNIV_CLKSRC_ID));

	//set counter value to 0
	writel(0, timer_base + TIMER_CNTVAL_REG(SUNIV_CLKSRC_ID));

	data = readl(timer_base + TIMER_CTL_REG(SUNIV_CLKSRC_ID));
	data |= (0x01 << 2) | (0x01 << 1) | (0x01 << 0);	//continuse mode, no pre-scale, 24M clock src, auto reload, enable 
	writel(data, timer_base + TIMER_CTL_REG(SUNIV_CLKSRC_ID));
	
	/* calculate the mult by shift  */
	suniv_clocksrc.mult = clocksource_hz2mult(SUNIV_HRES_CLK_RATE, suniv_clocksrc.shift);

	/* register clock source */
	clocksource_register(&suniv_clocksrc);
	/* set sched clock */
	setup_sched_clock(suniv_sched_clock_read, 32, SUNIV_HRES_CLK_RATE);
}
#endif
