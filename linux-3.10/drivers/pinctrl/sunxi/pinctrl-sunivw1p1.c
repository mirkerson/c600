/*
 * Allwinner SoCs(sunivw1p1) pinctrl driver.
 *
...
static const struct sunxi_desc_pin sunivw1p1_pins[] = {
...
static const struct sunxi_pinctrl_desc sunivw1p1_pinctrl_data = {
	.pins = sunivw1p1_pins,
	.npins = ARRAY_SIZE(sunivw1p1_pins),
	.pin_base = 0,
	.irq_banks = 3,
};

static int sunivw1p1_pinctrl_probe(struct platform_device *pdev)
{

	return sunxi_pinctrl_init(pdev,
				  &sunivw1p1_pinctrl_data);
}

static struct of_device_id sunivw1p1_pinctrl_match[] = {
	{ .compatible = "allwinner,sunivw1p1-pinctrl", },
	{}
};
MODULE_DEVICE_TABLE(of, sunivw1p1_pinctrl_match);

static struct platform_driver sunivw1p1_pinctrl_driver = {
	.probe	= sunivw1p1_pinctrl_probe,
	.driver	= {
		.name		= "sunivw1p1-pinctrl",
		.owner		= THIS_MODULE,
		.of_match_table	= sunivw1p1_pinctrl_match,
	},
};
static int __init sunivw1p1_pio_init(void)
{
	int ret;
	ret = platform_driver_register(&sunivw1p1_pinctrl_driver);
	if (IS_ERR_VALUE(ret)) {
		pr_debug("register sunivw1p1 pio controller failed\n");
		return -EINVAL;
	}
	return 0;
}
postcore_initcall(sunivw1p1_pio_init);

MODULE_AUTHOR("Jackie Hwang <huangshr@allwinnertech.com>");
MODULE_AUTHOR("Chen-Yu Tsai <wens@csie.org>");
MODULE_AUTHOR("Boris Brezillon <boris.brezillon@free-electrons.com");
MODULE_AUTHOR("Maxime Ripard <maxime.ripard@free-electrons.com");
MODULE_DESCRIPTION("Allwinner sunivw1p1 PIO pinctrl driver");
MODULE_LICENSE("GPL");
