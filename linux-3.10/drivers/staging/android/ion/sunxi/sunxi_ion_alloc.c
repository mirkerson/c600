...
extern struct ion_heap *g_carveout_heap;
/*
...
u32 inline __carveout_alloc(size_t size)
{
	...
	phys_addr = (u32)ion_carveout_allocate(g_carveout_heap, size, 0);
	return (u32)ION_CARVEOUT_ALLOCATE_FAIL != phys_addr ? phys_addr : 0;
}

void inline __carveout_free(u32 phys_addr, size_t size)
{
	ion_carveout_free(g_carveout_heap, (ion_phys_addr_t)phys_addr, size);
}
...
void sunxi_buf_free(void *vaddr, unsigned int paddr, unsigned int size)
{
	...
	ion_carveout_free(g_carveout_heap, (ion_phys_addr_t)paddr, size);
#endif
}
