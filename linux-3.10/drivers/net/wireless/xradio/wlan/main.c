...
#ifdef TCP_SIZE_ADJUST
extern u32 sysctl_wmem_max;
#endif
...
#ifdef TCP_SIZE_ADJUST
   hw_priv->tcp_max_snd_size = sysctl_wmem_max;
   /* Since we have a bad rtt in tcp layer, we need to
   * decrease the tcp send buf size
   */
   sysctl_wmem_max = 64 * 1024;
#endif
	...
	wsm_buf_init(&hw_priv->wsm_cmd_buf, (SDIO_BLOCK_SIZE<<1));
...
#ifdef TCP_SIZE_ADJUST
   sysctl_wmem_max = hw_priv->tcp_max_snd_size;
#endif
	for (i = 0; i < 4; ++i)
...
void xradio_core_deinit(void)
{
	...
	if (g_hw_priv) {
#ifdef HW_RESTART
		mutex_lock(&g_hw_priv->wsm_cmd_mux);
		g_hw_priv->exit_sync = true;
		mutex_unlock(&g_hw_priv->wsm_cmd_mux);
