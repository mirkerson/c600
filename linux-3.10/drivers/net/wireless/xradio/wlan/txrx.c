	...
	struct ieee80211_tx_rate *rates, size_t count)
{
	...
	if (unlikely(!(rates[0].flags & IEEE80211_TX_RC_MCS)) &&
		hw_priv->channel->band == IEEE80211_BAND_2GHZ) {
		u32 rateset = (priv->oper_rates | priv->base_rates) & ~0xf;
		if (rateset)
			lowest_rate_idx = __ffs(rateset);
		else
			lowest_rate_idx = 0xff;
		txrx_printk(XRADIO_DBG_NIY, "rateset=0x%x, lowest_rate_idx=%d\n",
	...
	if (WSM_CAPS_11N_TO_11BG(hw_priv->wsm_caps) && !priv->vif->p2p &&
		(priv->association_mode.basicRateSet & BIT(2)) &&
		((rates[0].flags & IEEE80211_TX_RC_MCS) || rates[count-1].idx > 2) &&
		rates[count-1].count >= 3) {
		u32 cnt1 = (rates[count-1].count>>2);
		u32 cnt2 = ((rates[count-1].count-cnt1)>>1);
		policy->tbl[0] |= ((cnt1&0xf));
		policy->tbl[0] |= ((cnt2&0xf)<<(2<<2));
		rates[count-1].count -= (cnt1 + cnt2);
		txrx_printk(XRADIO_DBG_NIY,
			"[TX policy]to11b=%d(f=%d),%d, cnt1=%d, cnt2=%d\n",
