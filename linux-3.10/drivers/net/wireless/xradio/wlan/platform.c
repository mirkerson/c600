...
static u32 wlan_reset_gpio = GPIOD(16);
static u32 wlan_irq_gpio   = GPIOD(13);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0))
extern void sunxi_mmc_rescan_card(unsigned id, unsigned insert);
#else
...
int xradio_sdio_detect(int enable)
{
	int insert = enable;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0))
	sunxi_mmc_rescan_card(wlan_bus_id, insert);
#else
