...
int __init lcd_module_init(void)
{
	...
	char *device_type = "lcd0";

	__inf("lcd_module_init\n");

	np = of_find_node_by_type(NULL,device_type);
	if (NULL == np) {
		pr_warning("%s: cannot found device type(%s) from dts, pls check\n", __func__, device_type);
		return -1;
	}

	ret = of_property_read_u32(np,"lcd_used", &lcd_used);
	if ((0 != ret) || (1 != lcd_used)) {
		pr_info("%s: lcd_used maybe not set, ret=%d, lcd_used=%d\n", __func__, ret, lcd_used);
		return 0;
	}
