...
void *disp_malloc(u32 num_bytes, u32 *phys_addr)
{
	...
	__inf("disp malloc memory size = 0x%x\n", num_bytes);
	if(num_bytes != 0) {
		actual_bytes = MY_BYTE_ALIGN(num_bytes);

		address = sunxi_buf_alloc(actual_bytes, phys_addr);
		if (address) {
			__inf("disp_malloc ok, virt_addr=0x%08x, phy_addr=0x%08x, size=0x%x\n", address, *phys_addr, num_bytes);
			return address;
		} else {
...
#else		//not define CONFIG_ION_SUNXI
...
void *disp_malloc(u32 num_bytes, u32 *phys_addr)
{
    ...
    *phys_addr = virt_to_phys(newptr->address);
...
__s32 DRV_DISP_Init(void)
{
	...
	for (mod_id = 0; mod_id < DISP_MOD_NUM; mod_id++) {
		para.reg_base[mod_id] = g_fbi.reg_base[mod_id];
		para.irq_no[mod_id] = g_fbi.irq_no[mod_id];
		para.mclk[mod_id] = g_fbi.mclk[mod_id];
	}
...
int disp_mem_request(int sel,__u32 size)
{
	...
	if(ret != 0)
	{
	    ...
	    memset(g_disp_mm[sel].info_base,0,size);
	    __inf("pa=0x%08lx, va=0x%08x, size:0x%x\n",g_disp_mm[sel].mem_start, g_disp_mm[sel].info_base, size);
...
int disp_mem_release(int sel)
{
    ...
    disp_free((void *)g_disp_mm[sel].info_base, g_disp_mm[sel].mem_start, g_disp_mm[sel].mem_len);
    memset(&g_disp_mm[sel],0,sizeof(struct info_mm));
#endif
...
static int __init disp_probe(struct platform_device *pdev)//called when platform_driver_register
{
	fb_info_t * info = NULL;
	u8 mod_id = 0;	
	char *disp_mod_name[DISP_MOD_NUM] = {"defe", "debe", "de_interlace", "lcdc"};
	//int ret = -EINVAL;
	...
	for (mod_id = 0; mod_id < DISP_MOD_NUM; mod_id++) {
		//get base address of register
		info->reg_base[mod_id] = (__u32)of_iomap(pdev->dev.of_node,mod_id);
		if (!info->reg_base[mod_id]) {
			__wrn("fail to get base reg of module(%s)\n", disp_mod_name[mod_id]);
			return -EINVAL;
		} else {
			__inf("get base address(=0x%8x) of module %s ok\n", info->reg_base[mod_id], disp_mod_name[mod_id]);
		}

		//get irq num of module
		info->irq_no[mod_id] = (__u32)irq_of_parse_and_map(pdev->dev.of_node,mod_id);
		if (!info->irq_no[mod_id]) {
			__wrn("fail to get irq num of module(%s)\n", disp_mod_name[mod_id]);
			return -EINVAL;
		} else {
			__inf("get irq num(=%d) of module %s ok\n", info->irq_no[mod_id], disp_mod_name[mod_id]);
		}

		//get mclk of module
		info->mclk[mod_id] = of_clk_get(pdev->dev.of_node,mod_id);
		if (IS_ERR(info->mclk[mod_id])) {
			__wrn("fail to get irq num of module(%s)\n", disp_mod_name[mod_id]);
			return -EINVAL;
		} else {
			__inf("get mclk(=0x%x) of module %s ok\n", (u32)info->mclk[mod_id], disp_mod_name[mod_id]);
		}
	}
...
long disp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    //----tv----
        ...
        case DISP_CMD_TV_NOTCH_FILTER_ON:
            ret =  BSP_disp_tv_notch_filter_enable(ubuffer[0], ubuffer[1]);
            break;
...
#if defined (CONFIG_ARCH_SUNIVW1)
...
static struct platform_driver disp_driver =
{
	.probe		= disp_probe,
	.remove		= disp_remove,
	//.suspend    = disp_suspend,		//FIXME
	//.resume    = disp_resume,		//FIXME
