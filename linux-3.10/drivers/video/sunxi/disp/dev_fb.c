...
__s32 parser_disp_init_para(__disp_init_t * init_para)
{
	...
	if(TYPE_INTEGER != OSAL_Script_Parser_Disp_Init_Data("disp_init", "disp_init_enable", &value, TYPE_INTEGER)) {
	...
	if(TYPE_INTEGER != OSAL_Script_Parser_Disp_Init_Data("disp_init", "disp_mode", &value, TYPE_INTEGER)) {
	...
	//screen0
	if(TYPE_INTEGER != OSAL_Script_Parser_Disp_Init_Data("disp_init", "screen0_output_type", &value, TYPE_INTEGER)) {
	...
	if(TYPE_INTEGER != OSAL_Script_Parser_Disp_Init_Data("disp_init", "screen0_output_mode", &value, TYPE_INTEGER)) {
	...
	//screen1
	if(TYPE_INTEGER != OSAL_Script_Parser_Disp_Init_Data("disp_init", "screen1_output_type", &value, TYPE_INTEGER)) {
	...
	if(TYPE_INTEGER != OSAL_Script_Parser_Disp_Init_Data("disp_init", "screen1_output_mode", &value, TYPE_INTEGER)) {
	...
	//fb0
	if(TYPE_INTEGER != OSAL_Script_Parser_Disp_Init_Data("disp_init", "fb0_framebuffer_num", &value, TYPE_INTEGER)){
		__wrn("fetch script data disp_init.fb0_framebuffer_num fail\n");
		return -1;
	}
	init_para->buffer_num[0]= value;

	if(TYPE_INTEGER != OSAL_Script_Parser_Disp_Init_Data("disp_init", "fb0_format", &value, TYPE_INTEGER)) {
		__wrn("fetch script data disp_init.fb0_format fail\n");
		return -1;
	}
	init_para->format[0]= value;

	if(TYPE_INTEGER != OSAL_Script_Parser_Disp_Init_Data("disp_init", "fb0_pixel_sequence", &value, TYPE_INTEGER)) {
		__wrn("fetch script data disp_init.fb0_pixel_sequence fail\n");
		return -1;
	}
	init_para->seq[0]= value;

	if(TYPE_INTEGER != OSAL_Script_Parser_Disp_Init_Data("disp_init", "fb0_scaler_mode_enable", &value, TYPE_INTEGER)) {
	...
	//fb1
	if(TYPE_INTEGER != OSAL_Script_Parser_Disp_Init_Data("disp_init", "fb1_framebuffer_num", &value, TYPE_INTEGER)) {
		__wrn("fetch script data disp_init.fb1_framebuffer_num fail\n");
		return -1;
	}
	init_para->buffer_num[1]= value;

	if(TYPE_INTEGER != OSAL_Script_Parser_Disp_Init_Data("disp_init", "fb1_format", &value, TYPE_INTEGER)){
		__wrn("fetch script data disp_init.fb1_format fail\n");
		return -1;
	}
	init_para->format[1]= value;

	if(TYPE_INTEGER != OSAL_Script_Parser_Disp_Init_Data("disp_init", "fb1_pixel_sequence", &value, TYPE_INTEGER)) {
		__wrn("fetch script data disp_init.fb1_pixel_sequence fail\n");
		return -1;
	}
	init_para->seq[1]= value;

	if(TYPE_INTEGER != OSAL_Script_Parser_Disp_Init_Data("disp_init", "fb1_scaler_mode_enable", &value, TYPE_INTEGER)) {
...
static int __init Fb_map_video_memory(struct fb_info *info)
{
...
#else        
	__u32 phy_addr = 0;
	info->screen_base = (char __iomem *)disp_malloc(info->fix.smem_len, &phy_addr);
	if (NULL == info->screen_base) {
		__wrn("fail to malloc memory, size=0x%x\n", info->fix.smem_len);
		return -ENOMEM;
	}
	...
	__inf("Fb_map_video_memory, pa=0x%08lx, va=0x%08lx, size:0x%x\n",info->fix.smem_start,  info->screen_base, info->fix.smem_len);
...
#else
    disp_free((void *)info->screen_base, info->fix.smem_start, info->fix.smem_len);
#endif
}
...
__s32 Display_Fb_Request(__u32 fb_id, __disp_fb_create_para_t *fb_para)
{
	...
	__inf("%s: xres=%d, yres=%d, buf_num=%d, bpp=%d\n", __func__, xres, yres, fb_para->buffer_num, info->var.bits_per_pixel);
...
#if (defined(FB_RESERVED_MEM) && !defined(CONFIG_ION_SUNXI))
/*
* fb_start : memory start address for display module, this address must be the same as the one in sunivw1p1.dtsi
* fb_size : memory size reserve for display module, this size must be the same as the one in sunivw1p1.dtsi
*/
unsigned long fb_start = 0x81c00000 ;
unsigned long fb_size = 0x00400000;	/*4M*/
#endif
...
{    
    ...
    {
#if (defined(FB_RESERVED_MEM) && !defined(CONFIG_ION_SUNXI))
        __inf("fbmem: fb_start=0x%08lx, fb_size=0x%08lx\n", fb_start, fb_size);
...
#if defined(CONFIG_SUNXI_BOOT_COLORBAR)
