...
static __u32 scaler_irq_no[2] = {0};
...
__s32 Scaler_Init(__u32 sel, __disp_bsp_init_para * para)
{
	scaler_clk_init(sel, para->mclk[DISP_MOD_DEFE]);
	DE_SCAL_EnableINT(sel,DE_WB_END_IE);

	scaler_irq_no[sel] = para->irq_no[DISP_MOD_DEFE];
	if(sel == 0)
	{
		OSAL_RegISR(scaler_irq_no[sel],0,Scaler_event_proc, (void *)sel,0,0);
#ifndef __LINUX_OSAL__
		OSAL_InterruptEnable(scaler_irq_no[sel]);
#endif
	}
	else if(sel == 1)
	{
...
__s32 Scaler_Exit(__u32 sel)
{
    if(sel == 0)
    {
        OSAL_InterruptDisable(scaler_irq_no[sel]);
        OSAL_UnRegISR(scaler_irq_no[sel],Scaler_event_proc,(void*)sel);
	scaler_irq_no[sel] = 0;
    }
    else if(sel == 1)
    {
