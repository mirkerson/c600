...
static __u32 lcdc_irq_no[2] = {0};

void LCD_get_reg_bases(__reg_bases_t *para)
{
	para->base_lcdc0 = gdisp.init_para.reg_base[DISP_MOD_LCDC];
...
__s32 LCD_get_panel_para(__u32 sel, __panel_para_t * info)
{
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_x", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_y", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_dclk_freq", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_pwm_used", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
		DE_WRN("fetch script data %s.lcd_pwm_used fail\n", primary_key);
	} else {
		info->lcd_pwm_used = value;
		DE_INF("lcd_pwm_used = %d\n", value);
	}

	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_pwm_ch", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_pwm_freq", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_pwm_pol", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_if", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_hbp", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_ht", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_vbp", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_vt", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_hv_if", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_hv_smode", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_hv_s888_if", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_hv_syuv_if", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_hv_vspw", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_hv_hspw", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_lvds_ch", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_lvds_mode", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_lvds_bitwidth", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_lvds_io_cross", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_cpu_if", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_frm", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_io_cfg0", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
		DE_WRN("fetch script data %s.lcd_io_cfg0 fail\n", primary_key);
	} else {
		info->lcd_io_cfg0 = value;
		DE_INF("lcd_io_cfg0 = 0x%08lx\n", value);
	}

	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_gamma_en", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
		DE_WRN("fetch script data %s.lcd_gamma_en fail\n", primary_key);
	} else {
	...
	if(info->lcd_gamma_correction_en) {
		for(i=0; i<256; i++)
		{
			...
			ret = OSAL_Script_FetchParser_Data(primary_key, name, &value, TYPE_INTEGER);
			if(TYPE_INTEGER != ret) {
...
void LCD_get_sys_config(__u32 sel, __disp_lcd_cfg_t *lcd_cfg)
{
	...
	//lcd_used
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_used", &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
		DE_WRN("%s.lcd_used not exist\n", primary_key);
		lcd_cfg->lcd_used = 0;
	} else {
		DE_INF("%s.lcd_used = %d\n", primary_key, value);
	...
	//lcd_bl_en	
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_bl_en_used", &value, TYPE_INTEGER);
	if (TYPE_INTEGER != ret) {
		DE_WRN("%s.lcd_bl_en_used \n", primary_key);
		lcd_cfg->lcd_bl_en_used = 0;
	} else {
		DE_INF("%s.lcd_bl_en_used = %d\n", primary_key, value);
		lcd_cfg->lcd_bl_en_used = value;
	}
	
	gpio_info = &(lcd_cfg->lcd_bl_en);
	ret = OSAL_Script_FetchParser_Data(primary_key,"lcd_bl_en", (int *)gpio_info, TYPE_GPIO);
	if(TYPE_GPIO != ret) {
		DE_INF("%s.lcd_bl_en not exist\n", primary_key);
		lcd_cfg->lcd_bl_en_used = 0;
	} else {
		DE_INF("%s.lcd_bl_en gpio_port=%d,gpio_name:%s, data:%d\n",primary_key, gpio_info->gpio, gpio_info->gpio_name, gpio_info->data);
		lcd_cfg->lcd_bl_en_used = 1;
	}

	//lcd_power
	/*
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_power_used", &value, TYPE_INTEGER);
	if (TYPE_INTEGER != ret) {
		DE_WRN("%s.lcd_power_used not exist\n", primary_key);
		lcd_cfg->lcd_power_used = 0;
	} else {
		DE_INF("%s.lcd_power_used = %d\n", primary_key, value);
		lcd_cfg->lcd_power_used = value;
	}
	*/
		
	gpio_info = &(lcd_cfg->lcd_power);
	ret = OSAL_Script_FetchParser_Data(primary_key,"lcd_power", (int *)gpio_info, TYPE_GPIO);
	if(TYPE_GPIO != ret) {
		DE_INF("%s.lcd_power not exist\n", primary_key);
		lcd_cfg->lcd_power_used= 0;
	} else {
		DE_INF("%s.lcd_power gpio_port=%d,gpio_name:%s, data:%d\n",primary_key, gpio_info->gpio, gpio_info->gpio_name, gpio_info->data);
	...
	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_pwm_used", &value, TYPE_INTEGER);
	if (TYPE_INTEGER != ret) {
		DE_WRN("%s.lcd_pwm_used not exist\n", primary_key);
		lcd_cfg->lcd_pwm_used = 0;
	} else {
		DE_INF("%s.lcd_pwm_used = %d\n", primary_key, value);
		lcd_cfg->lcd_pwm_used = value;
	}
		

	gpio_info = &(lcd_cfg->lcd_pwm);
	ret = OSAL_Script_FetchParser_Data(primary_key,"lcd_pwm", (int *)gpio_info, TYPE_GPIO);
	if(TYPE_GPIO != ret) {
		DE_INF("%s.lcd_pwm not exist\n", primary_key);
		lcd_cfg->lcd_pwm_used = 0;
	} else {
		DE_INF("%s.lcd_pwm gpio_port=%d,gpio_name:%s, data:%d\n",primary_key, gpio_info->gpio, gpio_info->gpio_name, gpio_info->data);
		lcd_cfg->lcd_pwm_used = 1;
	}


	//lcd io
	//FIXME: init lcd port as default state

	//init_bright
	/*
	sprintf(primary_key, "disp_init");
	sprintf(sub_name, "lcd%d_bright", sel);
	ret = OSAL_Script_Parser_Disp_Init_Data(primary_key, sub_name, &value, TYPE_INTEGER);
	if(TYPE_INTEGER != ret) {
...
__s32 LCD_PWM_EN(__u32 sel, __bool b_en)
{
    if(gdisp.screen[sel].lcd_cfg.lcd_pwm_used)
    {
        ...
        if((OSAL_sw_get_ic_ver() != 0xA) && (gpanel_info[sel].lcd_pwm_used))
        {
            if(b_en)
            {
...
{   
	...
	sprintf(dev_name, "lcd%d", sel);
	if(out_type == DISP_OUTPUT_TYPE_LCD)
	{
		state = (1== bon)? DISP_PIN_STATE_ACTIVE:DISP_PIN_STATE_SLEEP;
		ret = OSAL_Set_Disp_Port_State(dev_name, state);
		if (0 != ret) {
			pr_warn("%s: fail to set %s's pin to %s state, ret=%d\n", __func__, dev_name, state, ret);
		} else {
			__inf("%s: success to set %s's pin to %s state\n", __func__, dev_name, state);
		}
...
__s32 Disp_lcdc_init(__u32 sel, __disp_bsp_init_para * para)
{
	lcdc_irq_no[sel] = para->irq_no[DISP_MOD_LCDC];
    LCD_get_sys_config(sel, &(gdisp.screen[sel].lcd_cfg));		

    lcdc_clk_init(sel, para->mclk[DISP_MOD_LCDC]);
    //lvds_clk_init();		//FIXME
    ...
    if(sel == 0)
    {        
        OSAL_RegISR(lcdc_irq_no[sel],0,Disp_lcdc_event_proc,(void*)sel,0,0);
#ifndef __LINUX_OSAL__
        OSAL_InterruptEnable(lcdc_irq_no[sel]);
...
{	
    if(sel == 0)
    {
        OSAL_InterruptDisable(lcdc_irq_no[sel]);
        OSAL_UnRegISR(lcdc_irq_no[sel],Disp_lcdc_event_proc,(void*)sel);
    }
    else if(sel == 1)
    {
        OSAL_InterruptDisable(lcdc_irq_no[sel]);
