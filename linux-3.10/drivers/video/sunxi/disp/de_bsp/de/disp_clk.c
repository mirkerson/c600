...
static struct clk *g_scaler_clk = NULL;
static struct clk *g_image_clk = NULL;
static struct clk *g_lcdc_clk = NULL;
...
__s32 image_clk_init(__u32 sel, struct clk *clk)
{
	//__u32 dram_pll;
	__s32 ret = -1;
	...
	if(sel == 0)
	{
		ret = clk_prepare_enable(g_image_clk);
...
__s32 image_clk_on(__u32 sel)
{
	if(sel == 0)
	{
		...
		clk_prepare_enable(g_image_clk);
...
__s32 scaler_clk_init(__u32 sel, struct clk *clk)
{
	__s32 ret = -1;
	...
	if(sel == 0)
	{
		ret = clk_prepare_enable(g_scaler_clk);		
...
__s32 lcdc_clk_init(__u32 sel,  struct clk *clk)
{
	__s32 ret = -1;
	...
	if(sel == 0)
	{
		ret = clk_prepare_enable(g_lcdc_clk);
...
static __s32 LCD_PLL_Calc(__u32 sel, __panel_para_t * info, __u32 *divider)
{
	...
	__inf("%s: lcd_dclk_freq=%d\n", __func__, info->lcd_dclk_freq);
	...
	__inf("%s: lcd_dclk_freq=%d, div=%d, pll_freq=%d\n", __func__, info->lcd_dclk_freq, *divider, pll_freq);
	return pll_freq;
}
					...
					__u32 lcd_clk_div, __u32 hdmi_freq, __u32 pll_2x, __u32 type)
{
	...
	{		
		...
		if(gpanel_info[sel].tcon_index == 0)	//tcon0 drive lcd panel
		{
			...
			__inf("%s: set tcon0 dclk div to %d\n", __func__, lcd_clk_div);
	...
	{	    
		...
		#else
			pr_info("%s: tv/vga/hdmi clk setting is not support yet!\n", __func__);
		#endif
	}
