...
typedef union
{
	...
	struct
	{
		u32 defe_bypass_en               :  1 ;    /*scale by-pass control, this bit only can be set when input size equals to the output size*/ 
...
typedef union
{
	...
	struct
	{
		u32 en                           :  1 ;    // Default: 0x0; EN , Scaler enable, 0: Disable, 1: Enable, When scaler enable bit 
                                                   ...
                                                   // 
		u32 res0                         : 30 ;    // Default: ; / 
		u32 bist_en			: 1;	/*bist enable for cpu, 0: disable cpu accessing ram, mux ram to DEFE, default 0*/
	} bits;
} SCAL_EN_REG;
...
typedef union
{
	...
	struct
	{
		...
		u32 frm_start                    :  1 ;    // Default: 0x0; FRM_START , Frame start & reset control, 0: reset, 1: start, , If 
                                                   ...
                                                   // 
		u32 res3                         : 6 ;    // Default: ; / 
		u32 coef_access_ctrl	:1; /*fir coef ram access control, this bit will set to 1 before cpu access fir coef ram*/
		u32 res4                          :8;
	} bits;
} SCAL_FRM_CTRL_REG;
...
typedef union
{
	...
	struct
	{
		...
		u32 res2                         : 7 ;    // Default: ; / 
		u32 wb_ch_sel                : 2;   /*write back channel select, 0/1: ch3; 2:ch4; 3:ch5*/
		u32 res3                           :14;
	} bits;
} SCAL_OUTPUT_FMT_REG;
...
typedef union
{
	...
	struct
	{
		...
		u32 res1                         :  5 ;    // Default: ; / 
		u32 coef_access_status : 1;  /*Fir coef access status, this bit must be 1 before cpu acces fir coef ram. when this bit is 1, scaler module will fetch 0x00004000 from ram*/
