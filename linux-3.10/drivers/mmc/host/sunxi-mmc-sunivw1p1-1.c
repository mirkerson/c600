...
#include "sunxi-mmc.h"
#include "sunxi-mmc-sunivw1p1-1.h"
...
static void sunxi_mmc_set_clk_dly(struct sunxi_mmc_host *host,int clk,int bus_width,int timing)
{
	...
	{
		void __iomem *ccmu_sd1 =  ioremap(0x1c20000+0x8c, 0x100);
		writel(readl(ccmu_sd1)& (~(7<<8)),ccmu_sd1);
		writel(readl(ccmu_sd1)& (~(7<<20)),ccmu_sd1);
		writel(readl(ccmu_sd1)|(3<<8),ccmu_sd1);
		writel(readl(ccmu_sd1)|(4<<20),ccmu_sd1);
		iounmap(ccmu_sd1);
	}
#else
