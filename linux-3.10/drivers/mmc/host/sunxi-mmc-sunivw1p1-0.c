...
#include "sunxi-mmc.h"
#include "sunxi-mmc-sunivw1p1-0.h"
...
static void sunxi_mmc_set_clk_dly(struct sunxi_mmc_host *host,int clk,int bus_width,int timing)
{
	...
	{
		void __iomem *ccmu_sd0 =  ioremap(0x1c20000+0x88, 0x100);
		writel(readl(ccmu_sd0) & (~(7<<8)),ccmu_sd0);
		writel(readl(ccmu_sd0) & (~(7<<20)),ccmu_sd0);
		writel(readl(ccmu_sd0) | (3<<8),ccmu_sd0);
		writel(readl(ccmu_sd0) | (4<<20),ccmu_sd0);
		iounmap(ccmu_sd0);
	}
#else
