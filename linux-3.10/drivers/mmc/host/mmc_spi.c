	...
	unsigned long timeout)
{
	...
	tx_buf_tmp = kmalloc(t->len, GFP_KERNEL);
	if (!tx_buf_tmp){
		printk("tx_buf_tmp kmalloc err\n");
		return -EINVAL;
	}
	memset(tx_buf_tmp,0,t->len);
	memcpy(tx_buf_tmp,(u8 *)t->tx_buf,t->len);
	if (host->mmc->use_spi_crc){
		scratch->crc_val = cpu_to_be16(
				crc_itu_t(0, t->tx_buf, t->len));
        tx_add_tmp = (u8 *)t->tx_buf;
     }
	...
	//printk("mmc_spi_writeblock end111\n");
    ...
    //printk("mmc_spi_writeblock end222\n");
		...
		struct mmc_data *data, u32 blk_size)
{
	...
	//printk("mmc_spi_data_do clock_rate =%u\n",clock_rate);
