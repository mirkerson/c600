...
#define     UBOOT_PARTITION_SIZE	(512 * 1024)
#define     MBR_SIZE			    	(16 * 1024)
#define	MBR_OFFSET				(UBOOT_PARTITION_SIZE - MBR_SIZE)
#define     DL_SIZE					(16 * 1024)
#define   	MBR_MAGIC			    "softw411"
#define     MBR_MAX_PART_COUNT		120
#define     MBR_RESERVED          	(MBR_SIZE - 32 - (MBR_MAX_PART_COUNT * sizeof(PARTITION)))   //mbr???????
#define 	NOR_BLK_SIZE			512

/* partition information */
typedef struct sunxi_partition_t
{
	unsigned  int       addrhi;				//??????, ?????????¦Λ
	unsigned  int       addrlo;				//
	unsigned  int       lenhi;				//????
	unsigned  int       lenlo;				//
	unsigned  char      classname[16];		//???υτ??
	unsigned  char      name[16];			//???υτ??
	unsigned  int       user_type;          //???????
	unsigned  int       keydata;            //????????????????????
	unsigned  int       ro;                 //??§Υ????
	unsigned  char      reserved[68];		//??????????????????128???
...
typedef struct
{
	...
	unsigned  int       version;			        // ?·Ϊ????? 0x00000100
...
static int partitions_register(struct mtd_info *mtd, struct mtd_part_parser_data *ppdata)
{
	...
	for (i=0; i<sunxi_mbr->PartCount-1; i++) {
		...
		partitions[0].size   = UBOOT_PARTITION_SIZE;
	...
	M25_DBG("partion register ret=%d\n", ret);
