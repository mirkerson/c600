...
static int csi_clk_get(struct csi_dev *dev)
{
...
#if 0
	if(dev->clock[CSI_MASTER_CLK] && dev->clock[CSI_MASTER_CLK_PLL_SRC]) {
		if (clk_set_parent(dev->clock[CSI_MASTER_CLK], dev->clock[CSI_MASTER_CLK_PLL_SRC])) {
			vfe_err("sclk src Name:%s, csi core clock set parent failed \n",dev->clock[CSI_MASTER_CLK_PLL_SRC]->name);
			return -1;
		}
		if (clk_set_rate(dev->clock[CSI_MASTER_CLK], CSI_CORE_CLK_RATE)) {
	...
	vfe_dbg(0,"csi core clk = %ld\n",clk_get_rate(dev->clock[CSI_MASTER_CLK]));
#endif
#endif
	return 0;
}
...
static int csi_clk_enable(struct csi_dev *dev)
{
	...
	int ret = 0;
	int i;
	for(i = 0; i < CSI_MASTER_CLK_PLL_SRC; i++)
	{
		if(CSI_MASTER_CLK != i) {
			if(dev->clock[i]) {
...
{
#ifdef VFE_CLK
	sunxi_periph_reset_assert(dev->clock[CSI_MASTER_CLK]);
#endif
}
...
{
#ifdef VFE_CLK
	sunxi_periph_reset_deassert(dev->clock[CSI_MASTER_CLK]);
#endif
}
...
static long sunxi_csi_subdev_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_SUNXI_CSI_SET_CORE_CLK:
		mutex_lock(&csi->subdev_lock);
		ret = clk_set_rate(csi->clock[CSI_MASTER_CLK], *(unsigned long *)arg);
		vfe_print("Set csi core clk = %ld, after Set csi core clk = %ld \n", *(unsigned long *)arg, clk_get_rate(csi->clock[CSI_MASTER_CLK]));
		mutex_unlock(&csi->subdev_lock);
		break;
