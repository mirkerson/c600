...
static int vidioc_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	pr_info("%s: read csi irq status=0x%x\n", __func__, readl(0xf1cb0034));
...
//	dev->jiffies = jiffies;
	...
	writel(0x7777777, CSI_PIN1_REG);	//disable csi pin 1

	val = readl(CSI_PIN2_REG) & 0xffff0000;	//disable csi pin 2
	val |= 0x7777;
	writel(val, CSI_PIN2_REG);

	pr_info("%s: disable csi pin\n", __func__);
streamoff_unlock:
	mutex_unlock(&dev->stream_lock);
...
//	writel(0x01, INTC_FF_REG1);
	
	ret = request_irq(dev->irq, vfe_isr, 0, "csi_irq", dev);
	if (ret) {
			vfe_err("i=%d, failed to install irq (%d)\n", i, ret);
				return -ENXIO;
	}	

	//writel(0x01, INTC_EN_IRQ1);
	writel(CSI_INT_ALL, CSI_IRQ_EN);	//enable all csi irq
	writel(0xff, CSI_BASE_REG);			//enable csi module

	pr_info("%s: csi irq(%d) enable\n", __func__, dev->irq);

	//writel(0xff, INTC_FF_REG1);
...
{	
	...
	if (i2c_adap == NULL) 
	{
		vfe_err("request i2c adapter failed, twi_id=%d!\n", ccm_cfg->twi_id);
		return -EFAULT;
	}
...
//	v4l2_subdev_call(dev->csi_sd, core, s_power, 1);   //调用s_power?
//	v4l2_subdev_call(dev->csi_sd, core, init, 1);      //调用init?
//	v4l2_subdev_call(dev->csi_sd, core, ioctl, 1, 1);  //调用ioctl?	
