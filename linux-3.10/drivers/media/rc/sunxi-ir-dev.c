...
static void ir_clk_cfg(void)
{
	...
	void __iomem *cir = ioremap(0x01c20000+0xB8, 0x100);
	writel(readl(cir)|(1<<24),cir);
	iounmap(cir);
...
static int sunxi_ir_startup(struct platform_device *pdev)
{
	...
	printk("pdev->dev.of_node name :%s\n", pdev->dev.of_node->name);
	...
	ir_data->mclk = devm_clk_get(&pdev->dev,"cir");
	if (NULL==ir_data->pclk||IS_ERR(ir_data->pclk)
