...
{
	u32 reg_val = 0;
	u32 div_clk = 200000000 / (spi_clk << 1);
...
/* start spi transfer */ //added by suwenrong for rapid mode
static void spi_set_rpsm(void __iomem *base_addr)
{
	u32 reg_val = readl(base_addr + SPI_TC_REG);
	reg_val |= SPI_TC_RPSM;
	writel(reg_val, base_addr + SPI_TC_REG);
...
static int sunxi_spi_prepare_ofdma(struct device *dev,spi_dma_info_t *_info,enum spi_dma_dir _dir)
{
	SPI_DBG("Init DMA, dir %d \n", _dir);
	
	if (_info->chan == NULL) {
		_info->chan = dma_request_slave_channel(dev,(const char *)((_dir == DMA_TO_DEVICE)?"tx":"rx"));
		if (_info->chan == NULL) {
			SPI_ERR("Request DMA(dir %d) failed!\n", _dir);
			return -EINVAL;
		}
...
static int sunxi_spi_config_dma_tx(struct sunxi_spi *sspi, struct spi_transfer *t)
{
	...
	if (!dma_desc) {
		SPI_ERR("[spi-%d]dmaengine_prep_slave_sg() failed!\n", sspi->master->bus_num);
		dma_unmap_sg(&sspi->pdev->dev, sspi->dma_tx.sg, sspi->dma_tx.nents, DMA_TO_DEVICE);
		return -1;
	}
...
static int sunxi_spi_xfer_setup(struct spi_device *spi, struct spi_transfer *t)
{
	...
	*/
	...
	//spi_set_rpsm(base_addr);//added by suwenrong
	...
	*/
	...
	if(t->len > BULK_DATA_BOUNDARY) {
		...
		case SINGLE_HALF_DUPLEX_RX:
		{
			...
			ret = sunxi_spi_prepare_ofdma(&sspi->pdev->dev,&sspi->dma_rx, SPI_DMA_RDEV);
			if(ret < 0) {
		...
		case SINGLE_HALF_DUPLEX_TX:
		{
			...
			ret = sunxi_spi_prepare_ofdma(&sspi->pdev->dev,&sspi->dma_tx, SPI_DMA_WDEV);
			if(ret < 0) {
