...
static u32 debug_mask = 0;//DEBUG_SUSPEND | DEBUG_DATA_INFO | DEBUG_CONTROL_INFO | DEBUG_INIT;
...
static void report_abs(void)
{
	...
	dprintk(DEBUG_DATA_INFO, "x= %d y= %d z= %d\n", x, y, z);
}
...
static void polled_dev_open(struct input_polled_dev *dev)
{
	...
	dprintk(DEBUG_INIT, "polled_dev_open\n");
	mutex_lock(&mma7660_data.init_mutex);
...
static void polled_dev_close(struct input_polled_dev *dev)
{
	...
	dprintk(DEBUG_INIT, "polled_dev_close\n");
	mutex_lock(&mma7660_data.init_mutex);
	error = i2c_smbus_write_byte_data(mma7660_i2c_client, MMA7660_MODE,
...
//	assert(!(IS_ERR(hwmon_dev)));
	...
	mma7660_idev->open= polled_dev_open;
	mma7660_idev->close= polled_dev_close;
