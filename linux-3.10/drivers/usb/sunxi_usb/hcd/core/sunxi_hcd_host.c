...
static void sunxi_hcd_h_ep0_flush_fifo(struct sunxi_hcd_hw_ep *ep)
{
	...
	old_ep_index = USBC_GetActiveEp(ep->sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle);
	USBC_SelectActiveEp(ep->sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle, ep->epnum);
	...
	do {
		...
		USBC_Host_FlushFifo(ep->sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle, USBC_EP_TYPE_EP0);
	...
	USBC_SelectActiveEp(ep->sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle, old_ep_index);
 ...
 */
static void sunxi_hcd_clean_ep_dma_status_and_flush_fifo(struct sunxi_hcd_qh *qh)
{
	struct sunxi_hcd_hw_ep   *hw_ep = NULL;
	u8  	old_ep_index 	= 0;
	__hdle 	usb_bsp_hdle 	= 0;
...
static inline void sunxi_hcd_h_tx_dma_start(struct sunxi_hcd_hw_ep *ep, struct sunxi_hcd_qh *qh)
{
	void __iomem 	*usbc_base 	= ep->regs;
	u8		txcsr 		= 0;
	u16 		old_ep_index 	= 0;

	old_ep_index = USBC_GetActiveEp(qh->hw_ep->sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle);
	USBC_SelectActiveEp(qh->hw_ep->sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle, qh->hw_ep->epnum);
	...
	USBC_SelectActiveEp(qh->hw_ep->sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle, old_ep_index);
	return;
}
							...
							u32 length)
{
	...
	/* config dma */
	sunxi_hcd_dma_set_config(qh, (dma_addr_t)urb->transfer_buffer + offset, length);
	sunxi_hcd_dma_start(qh, (u32 __force)hw_ep->fifo, (dma_addr_t)urb->transfer_buffer + offset, length);
	//sunxi_hcd_dma_set_config(qh, (urb->transfer_dma + offset), length);
	//sunxi_hcd_dma_start(qh, (u32)hw_ep->fifo, (urb->transfer_dma + offset), length);
 ...
 *
 */
static void sunxi_hcd_start_urb(struct sunxi_hcd *sunxi_hcd, int is_in, struct sunxi_hcd_qh *qh)
{
	u16                 frame       = 0;
	u32                 len         = 0;
	void __iomem        *usbc_base  = NULL;
	...
	default:
start:
		if (is_sunxi_hcd_dma_capable(sunxi_hcd->usbc_no, len, qh->maxpacket, epnum)) {
 ...
 *
 */
static inline void sunxi_hcd_save_toggle(struct sunxi_hcd_hw_ep *ep, int is_in, struct urb *urb)
{
 ...
 *
 */
static struct sunxi_hcd_qh *sunxi_hcd_giveback(struct sunxi_hcd_qh *qh, struct urb *urb, int status)
{
	...
	if (list_empty(&qh->hep->urb_list)) {
		...
		case USB_ENDPOINT_XFER_INT:
			...
			if(qh->type == USB_ENDPOINT_XFER_ISOC){
				...
				sunxi_hcd_ep_select(sunxi_hcd->mregs, qh->hw_ep->epnum);
							...
							int is_in)
{
	...
	if (!is_host_active(sunxi_hcd) || !sunxi_hcd->is_active) {
		printk("sunxi_hcd_advance_schedule, host is not active\n");
		return ;
	}
 ...
 *
 */
static bool sunxi_hcd_host_packet_rx(struct sunxi_hcd *sunxi_hcd, struct urb *urb, u8 epnum, u8 iso_err)
{
	u16			rx_count    = 0;
	u8			*buf        = NULL;
	u16			csr         = 0;
	...
	if (usb_pipeisoc(pipe)) {
		...
		if(DOBULE_EP)
		{
			...
			if((hw_ep->epnum == BAND_WIDTH_ISO_EP) || (hw_ep->epnum == BAND_WIDTH_ISO_EP_EX)){
				...
				other_csr &= ~((1 << USBC_BP_RXCSR_H_RX_PKT_READY) | (1 << USBC_BP_RXCSR_H_REQ_PACKET));
 ...
 */
static void sunxi_hcd_rx_reinit(struct sunxi_hcd *sunxi_hcd, struct sunxi_hcd_qh *qh, struct sunxi_hcd_hw_ep *ep)
{
	u16	csr = 0;
	void __iomem *usbc_base  = NULL;
	...
	if (ep->is_shared_fifo) {
		...
		sunxi_hcd_h_rx_flush_fifo(ep, (1 << USBC_BP_RXCSR_H_CLEAR_DATA_TOGGLE));
	}
...
#ifdef CONFIG_UDC_HIGH_BANDWITH_ISO
	USBC_Writew((qh->maxpacket|((qh->PacketCount)<<11)), USBC_REG_RXMAXP(usbc_base));
	if(qh->type == USB_ENDPOINT_XFER_ISOC){
						...
						u32 len)
{
	...
	DMSG_DBG_HCD("sunxi_hcd_ep_program: %s hw%d urb %p spd%d dev%d ep%d%s "
	...
	if (is_out) { /* tx */
		...
		/* target addr and (for multipoint) hub addr/port */
		if (sunxi_hcd->is_multipoint) {
			sunxi_hcd_write_txfunaddr(usbc_base, epnum, qh->addr_reg);
			sunxi_hcd_write_txhubaddr(usbc_base, epnum, qh->h_addr_reg);
			sunxi_hcd_write_txhubport(usbc_base, epnum, qh->h_port_reg);
		...
		/* dma transmit */
		if (is_sunxi_hcd_dma_capable(sunxi_hcd->usbc_no, len, packet_sz, epnum)) {
			if (sunxi_hcd_tx_dma_program(hw_ep, qh, urb, offset, len)) {
				load_count = 0;
			}
		}
		...
		if (is_sunxi_hcd_dma_capable(sunxi_hcd->usbc_no, len, packet_sz, epnum)) {
			...
			//sunxi_hcd_dma_set_config(qh, (urb->transfer_dma + offset), len)
			//sunxi_hcd_dma_start(qh, (u32)hw_ep->fifo, (urb->transfer_dma + offset), len);
 ...
 */
static bool sunxi_hcd_h_ep0_continue(struct sunxi_hcd *sunxi_hcd, u16 len, struct urb *urb)
{
	bool                    more        = false;
	u8                      *fifo_dest  = NULL;
	u16                     fifo_count  = 0;
...
void sunxi_hcd_host_tx(struct sunxi_hcd *sunxi_hcd, u8 epnum)
{
	...
	if (is_sunxi_hcd_dma_capable(sunxi_hcd->usbc_no, qh->segsize, qh->maxpacket, epnum)
		 ...
		 */
		if (tx_csr & ((1 << USBC_BP_TXCSR_H_FIFO_NOT_EMPTY) | (1 << USBC_BP_TXCSR_H_TX_READY))) {
			DMSG_ERR("ERR: sunxi_hcd_host_tx, DMA complete but packet still in FIFO, "
...
static void sunxi_hcd_bulk_rx_nak_timeout(struct sunxi_hcd *sunxi_hcd, struct sunxi_hcd_hw_ep *ep)
{
	...
	if (cur_qh) {
		...
		if (sunxi_hcd_dma_is_busy(cur_qh)) {
			//urb->actual_length += sunxi_hcd_dma_transmit_length(cur_qh, is_direction_in(cur_qh), (urb->transfer_dma + urb->actual_length));
...
void sunxi_hcd_host_rx(struct sunxi_hcd *sunxi_hcd, u8 epnum)
{
	...
	if ((urb == NULL) || (u32)urb < 0x7fffffff) {
		printk("sunxi_hcd_host_rx, urb is %p\n", urb);
		return ;
	}
	...
	if (status) {
		...
		if (sunxi_hcd_dma_is_busy(qh)) {
			...
			sunxi_hcd_dma_stop(qh);
			sunxi_hcd_clean_ep_dma_status_and_flush_fifo(qh);
			//xfer_len = sunxi_hcd_dma_transmit_length(qh, is_direction_in(qh), (urb->transfer_dma + urb->actual_length));
	...
	if (is_sunxi_hcd_dma_capable(sunxi_hcd->usbc_no, qh->segsize, qh->maxpacket, epnum) && (rx_csr & (1 << USBC_BP_RXCSR_H_DMA_REQ_EN))) {
		...
		if (unlikely(!(rx_csr & (1 << USBC_BP_RXCSR_H_RX_PKT_READY)))) {
			...
			printk("Rx interrupt with no errors or packet!\n");
				...
				int is_in)
{
	...
	for (epnum = 1, hw_ep = sunxi_hcd->endpoints + 1;
		...
		//	      epnum, sunxi_hcd->nr_endpoints, hw_ep->in_qh, hw_ep->out_qh);

#ifdef CONFIG_UDC_HIGH_BANDWITH_ISO
		if(epnum == BAND_WIDTH_ISO_EP && qh->type == USB_ENDPOINT_XFER_ISOC){
			if(qh->maxpacket > 512){
				printk("HIGH BAND_WIDTH_ISO\n");
			}else{
				continue;
			}
		}
		...
		//printk("best_end = %d, (qh:0x%p,type:%d,max%d)\n", best_end, qh, qh->type, qh->maxpacket);
...
#endif
		//printk("hw_ep:%d, max:%d, %d\n",  hw_ep->epnum, hw_ep->max_packet_sz_rx, hw_ep->rx_reinit);
...
static int sunxi_hcd_cleanup_urb(struct urb *urb, struct sunxi_hcd_qh *qh, int is_in)
{
	...
	DMSG_INFO("sunxi_hcd_cleanup_urb: qh(0x%p,0x%x,0x%x), urb(0x%p,%d,%d), ep(0x%p,%d,0x%p,0x%p)\n",
		qh, qh->epnum, qh->type,
		urb, urb->transfer_buffer_length, urb->actual_length,
	...
	if (is_sunxi_hcd_dma_capable(ep->sunxi_hcd->usbc_no, urb->transfer_buffer_length, qh->maxpacket, ep->epnum)) {
		...
		sunxi_hcd_dma_stop(qh);
		sunxi_hcd_clean_ep_dma_status_and_flush_fifo(qh);
		//urb->actual_length += sunxi_hcd_dma_transmit_length(qh, is_direction_in(qh), urb->transfer_dma);
...
int sunxi_hcd_urb_dequeue(struct usb_hcd *hcd, struct urb *urb, int status)
{
	...
	DMSG_INFO("[sunxi_hcd]: sunxi_hcd_urb_dequeue, sunxi_hcd(%p, 0x%d, 0x%x),"
...
void sunxi_hcd_h_disable(struct usb_hcd *hcd, struct usb_host_endpoint *hep)
{
	...
	DMSG_INFO("[sunxi_hcd]: sunxi_hcd_h_disable, epnum = %x\n", epnum);
