...
static s32 usb_clock_init(sunxi_hcd_io_t *sunxi_hcd_io, struct platform_device *pdev)
{
...
#else
	UsbPhyInit(0, sunxi_hcd_io->usb_vbase);
#endif
...
static __s32 sunxi_hcd_io_init(__u32 usbc_no, struct platform_device *pdev, sunxi_hcd_io_t *sunxi_hcd_io)
{
	...
	reg_value = USBC_Readl(sunxi_hcd_io->sram_vbase + 0x04);	
	reg_value |= 0x01;	
	USBC_Writel(reg_value, (sunxi_hcd_io->sram_vbase + 0x04));
	...
	/* open usb lock */
	ret = usb_clock_init(sunxi_hcd_io, pdev);
	if (ret != 0) {
...
{
#if defined(CONFIG_USB_SUNXI_USB0_OTG)
	struct platform_device 	*pdev 	= NULL;
...
#ifndef CONFIG_USB_SUNXI_USB_MANAGER
...
#endif
#ifndef CONFIG_USB_SUNXI_USB_MANAGER
static struct usb_port_info h_port = {
	.enable = 1,
	.port_type = 1,
	.det_vbus = {
		.valid = 0,
...
#ifdef  CONFIG_USB_SUNXI_USB0_OTG
#ifndef CONFIG_USB_SUNXI_USB_MANAGER
	struct sunxi_hcd_platform_data	*pdata = &sunxi_hcd_plat;

	sunxi_hcd_plat.config->port_info = &h_port;
	pdev->dev.platform_data = &sunxi_hcd_plat;
	return sunxi_hcd_probe_otg(pdev);
#else
	struct sunxi_hcd_platform_data	*pdata = &sunxi_hcd_plat;//pdev->dev.platform_data;

	sunxi_hcd_plat.config->port_info = &(g_usb_cfg.port);
	pdev->dev.platform_data = &sunxi_hcd_plat;
