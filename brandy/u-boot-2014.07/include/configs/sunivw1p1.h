...
#ifdef CONFIG_SUNIVW1P1_NOR
#define CONFIG_TARGET_NAME      spinor-sunivw1p1
#else
#define CONFIG_TARGET_NAME      sunivw1p1
#endif
...
#define CONFIG_SYS_SRAM_SIZE             (0xA600) //40K  SRAM+BOOT_SRAM
#define CONFIG_SYS_SRAM_USB_BASE     (0x00008000)
#define CONFIG_SYS_SRAM_USB_SIZE      (0x00000800)   // 2K
...
#define PLAT_SDRAM_BASE                      	(0x80000000)//	0x40000000
//uboot run addr
#define CONFIG_SYS_TEXT_BASE                ( 0x80800000) // 0x4A000000
//dram base for uboot
#define CONFIG_SYS_SDRAM_BASE              PLAT_SDRAM_BASE
//book pkg addr
#define CONFIG_BOOTPKG_STORE_IN_DRAM_BASE    (CONFIG_SYS_SDRAM_BASE + 0x005c0000) //(CONFIG_SYS_SDRAM_BASE + 0x2e00000)
//fdt addr for kernel
#define CONFIG_SUNXI_FDT_ADDR                (CONFIG_SYS_SDRAM_BASE+0x03000000)// (CONFIG_SYS_SDRAM_BASE+0x04000000)
...
#define PHYS_SDRAM_1_SIZE			(32 << 20)	///(512 << 20)				/* 0x20000000, 512 MB Bank #1 */
 ...
 */
#define CONFIG_SYS_MALLOC_LEN		(CONFIG_ENV_SIZE + (20 << 20))//(CONFIG_ENV_SIZE + (16 << 20)) //(CONFIG_ENV_SIZE + (128 << 20))     //-------
...
#define SUNXI_DISPLAY_FRAME_BUFFER_ADDR   (CONFIG_SYS_SDRAM_BASE + 0x00c40000) // (CONFIG_SYS_SDRAM_BASE + 0x06400000)
#define SUNXI_DISPLAY_FRAME_BUFFER_SIZE  	 (0x0100000)//0x01000000
...
#define MEMCPY_TEST_SRC                   (CONFIG_SYS_SDRAM_BASE + 0x00c00000) //(CONFIG_SYS_SDRAM_BASE + 0x06000000)
...
#define CONFIG_BOOT0_RET_ADDR           (CONFIG_SYS_SRAM_BASE)	// (CONFIG_SRAM_BOOT_BASE)	//(CONFIG_SYS_SRAM_BASE)
#define CONFIG_BOOT0_RUN_ADDR           (0X0000)   // (CONFIG_SRAM_BOOT_BASE)//(0x00000)
...
#define CONFIG_SYS_MEMTEST_END		((CONFIG_SYS_SDRAM_BASE + 32)<<20) //((CONFIG_SYS_SDRAM_BASE + 256)<<20)	/* 256M */
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"init=${init} loglevel=${loglevel} partitions=${partitions}\0" \
	"boot_normal=sunxi_flash read 80007800 boot;boota 80007800\0" \
	"boot_recovery=sunxi_flash read 80007800 recovery;boota 80007800\0" \
