...
{
	board_late_init();
printf("%s %d-initr_sunxi_base-END--\n",__FILE__,__LINE__);	
	return 0;
}
...
//suwenrong add for recovery
#define BOOT_PARTITION_ADDR		        0x80000 //512k
#define SECTOR_SIZE 					0x200 //512
static void recover_nor_flash(void)
{       
    uint mtd1_addr, mtd2_addr, mtd5_addr, mtd6_addr;		       
    uint mtd1_size, mtd2_size, mtd5_size, mtd6_size;			   

	sunxi_partition_init(); 
	
	uint offset = BOOT_PARTITION_ADDR - 32*512;

	mtd1_addr = BOOT_PARTITION_ADDR; 
	mtd2_addr = SECTOR_SIZE * sunxi_partition_get_offset_byname("rootfs") + offset;
	mtd5_addr = SECTOR_SIZE * sunxi_partition_get_offset_byname("recoverk") + offset;
	mtd6_addr = SECTOR_SIZE * sunxi_partition_get_offset_byname("recoverf") + offset;

	mtd1_size = SECTOR_SIZE * sunxi_partition_get_size_byname("boot");	
	mtd2_size = SECTOR_SIZE * sunxi_partition_get_size_byname("rootfs");	
	mtd5_size = SECTOR_SIZE * sunxi_partition_get_size_byname("recoverk");	
	mtd6_size = SECTOR_SIZE * sunxi_partition_get_size_byname("recoverf");

    
    printf("*(uint32_t *)(0x81ff0000)=%d\n", *(uint32_t *)(0x81ff0000));
       
    char *buf, *tmp_buf;
    int i;
        
    if(11250816 == *(uint32_t *)(0x81ff0000))
    {        
        *(uint32_t *)(0x81ff0000) = 0;
        
        buf = (char*)malloc(0x300000);    
        if(NULL == buf)
        {
            printf("malloc fail!\n");
            return ;
        ...
        printf("\nread mtd5-------------\n");
        sunxi_flash_read( mtd5_addr/512 - CONFIG_SPINOR_LOGICAL_OFFSET, mtd5_size/512, buf); //recoverk
        
        printf("\nerase mtd1-------------\n");
        for(i = mtd1_addr/(64*1024); i < (mtd1_addr + mtd1_size)/(64*1024); i++)
        {
            sunxi_flash_erase(i);
        }

        printf("\nrecover mtd1-------------\n");            
        for(i = mtd1_addr/512; i < (mtd1_addr + mtd1_size)/512; i++)
        {
            sunxi_flash_write( i - CONFIG_SPINOR_LOGICAL_OFFSET, 1, tmp_buf );
            tmp_buf = tmp_buf + 512;
        }
        ...
        printf("\nread mtd6-------------\n");
        sunxi_flash_read( mtd6_addr/512 - CONFIG_SPINOR_LOGICAL_OFFSET, mtd6_size/512, buf); //recoveryf
   
        printf("\nerase mtd2-------------\n");
        for(i = mtd2_addr/(64*1024); i < (mtd2_addr + mtd2_size)/(64*1024); i++)
        {
            sunxi_flash_erase(i);
        }
        ...
        for(i = mtd2_addr/512; i < (mtd2_addr + mtd2_size)/512; i++)
        {
            sunxi_flash_write( i - CONFIG_SPINOR_LOGICAL_OFFSET, 1, tmp_buf );
            tmp_buf = tmp_buf + 512;
        }
...
void intr_vector_fix(void)
{
	...
	for(i =0; i < 8; i++)
	{
//		printf("before fix: vector %d, addr %lx ,value 0x%lx\n", i ,(ulong)vec_array[i], *(vec_array[i]));
		*(vec_array[i]) +=  (gd->reloc_off);
//		printf("after  fix: vector %d, addr %lx ,value 0x%lx\n", i ,(ulong)vec_array[i], *(vec_array[i]));
