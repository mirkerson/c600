...
#endif
//	printf("%s %d--working_fdt = %x , images.ft_addr = %d , fdt_size = %x\n",__FILE__,__LINE__, (uint)working_fdt, (uint)images.ft_addr,(uint)gd->fdt_size);
	return 0;
}
#endif
...
//printf("%s %d--load_buf = %x, image_buf = %x\n",__FILE__,__LINE__,(uint)load_buf, (uint)image_buf);
...
//printf("%s %d--boot_fn = 0x%x, images->os.os = %d, images->ep = %x,states =%x\n",__FILE__,__LINE__,(uint)boot_fn, (uint)images->os.os, (uint)images->ep,states);
