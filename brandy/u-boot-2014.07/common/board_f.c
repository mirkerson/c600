...
int i =0 ;
printf("-flags = %d\n", gd->flags);
printf("---brun---uboot_spare_head.boot_head-------\n");
printf("jump_instruction = %d\n",uboot_spare_head.boot_head.jump_instruction);
printf("magic = %s\n",uboot_spare_head.boot_head.magic);
printf("check_sum = %d\n",uboot_spare_head.boot_head.check_sum);
printf("align_size = %d\n",uboot_spare_head.boot_head.align_size);
printf("length = %d\n",uboot_spare_head.boot_head.length);
printf("uboot_length = %d\n",uboot_spare_head.boot_head.uboot_length);
printf("version = %s\n",uboot_spare_head.boot_head.version);
printf("------uboot_spare_head.boot_data-------\n");
for(i = 0 ; i< 32; i++)
	printf("dram_para[%d] = %x\n",i,uboot_spare_head.boot_data.dram_para[i]);
printf("run_clock= %d\n",uboot_spare_head.boot_data.run_clock);
printf("run_core_vol= %d\n",uboot_spare_head.boot_data.run_core_vol);
printf("uart_port= %d\n",uboot_spare_head.boot_data.uart_port);
for(i = 0; i< 2; i++)
{
	printf("uart_gpio[%d].port= %d\n",i,uboot_spare_head.boot_data.uart_gpio[i].port);
	printf("uart_gpio[%d].port_num= %d\n",i,uboot_spare_head.boot_data.uart_gpio[i].port_num);
	printf("uart_gpio[%d].mul_sel= %d\n",i,uboot_spare_head.boot_data.uart_gpio[i].mul_sel);
	printf("uart_gpio[%d].pull= %d\n",i,uboot_spare_head.boot_data.uart_gpio[i].pull);
	printf("uart_gpio[%d].drv_level= %d\n",i,uboot_spare_head.boot_data.uart_gpio[i].drv_level);
	printf("uart_gpio[%d].data= %d\n",i,uboot_spare_head.boot_data.uart_gpio[i].data);
}
printf("twi_port= %d\n",uboot_spare_head.boot_data.twi_port);
for(i = 0; i< 2; i++)
{
	printf("twi_gpio[%d].port= %d\n",i,uboot_spare_head.boot_data.twi_gpio[i].port);
	printf("twi_gpio[%d].port_num= %d\n",i,uboot_spare_head.boot_data.twi_gpio[i].port_num);
	printf("twi_gpio[%d].mul_sel= %d\n",i,uboot_spare_head.boot_data.twi_gpio[i].mul_sel);
	printf("twi_gpio[%d].pull= %d\n",i,uboot_spare_head.boot_data.twi_gpio[i].pull);
	printf("twi_gpio[%d].drv_level= %d\n",i,uboot_spare_head.boot_data.twi_gpio[i].drv_level);
	printf("twi_gpio[%d].data= %d\n",i,uboot_spare_head.boot_data.twi_gpio[i].data);
}
printf("work_mode= %d\n",uboot_spare_head.boot_data.work_mode);
printf("storage_type= %d\n",uboot_spare_head.boot_data.storage_type);
printf("secureos_exist= %d\n",uboot_spare_head.boot_data.secureos_exist);
printf("uboot_start_sector_in_mmc= %x\n",uboot_spare_head.boot_data.uboot_start_sector_in_mmc);
printf("dtb_offset= %x\n",uboot_spare_head.boot_data.dtb_offset);
printf("boot_package_size= %x\n",uboot_spare_head.boot_data.boot_package_size);
printf("---------------------------------\n");
#endif
	...
	debug_gan("U-Boot code: %08X -> %08lX  BSS: -> %08lX\n",
	      CONFIG_SYS_TEXT_BASE, bss_start, bss_end);
#endif

#ifdef CONFIG_MODEM_SUPPORT
	debug_gan("Modem Support enabled\n");
#endif
...
static int show_dram_config(void)
{
	...
	for (i = size = 0; i < CONFIG_NR_DRAM_BANKS; i++) {
		...
		debug_gan("Bank #%d: %08lx ", i, gd->bd->bi_dram[i].start);
#ifdef DEBUG
...
static int setup_fdt(void)
{
	...
	void* fdt_head;
	fdt_head = (void*)(uboot_spare_head.boot_data.dtb_offset+CONFIG_SYS_TEXT_BASE);
	if(uboot_spare_head.boot_data.dtb_offset != 0)
	{
		gd->fdt_blob = (void*)(CONFIG_SYS_TEXT_BASE +uboot_spare_head.boot_head.length);
		memmove((void*)gd->fdt_blob, fdt_head, fdt_totalsize(fdt_head));
...
{
	debug_gan("Monitor len: %08lX\n", gd->mon_len);
	/*
	 * Ram is setup, size stored in gd !!
	 */
	debug_gan("Ram size: %08lX\n", (ulong)gd->ram_size);
	...
	if (gd->relocaddr > determine_mp_bootpg(NULL)) {
		...
		debug_gan("Reserving MP boot page to %08lx\n", gd->relocaddr);
	}
#endif
	return 0;
}
...
static int reserve_logbuffer(void)
{
	...
	debug_gan("Reserving %dk for kernel logbuffer at %08lx\n", LOGBUFF_LEN,
		gd->relocaddr);
	return 0;
}
#endif
...
static int reserve_pram(void)
{
	...
	debug_gan("Reserving %ldk for protected RAM at %08lx\n", reg,
	      gd->relocaddr);
	return 0;
}
#endif /* CONFIG_PRAM */
...
static int reserve_mmu(void)
{
	...
	debug_gan("TLB table from %08lx to %08lx\n", gd->arch.tlb_addr,
...
static int reserve_trace(void)
{
	...
	debug_gan("Reserving %dk for trace data at: %08lx\n",
...
static int reserve_uboot(void)
{
	...
	debug_gan("Reserving %ldk for U-Boot at: %08lx\n", gd->mon_len >> 10,
	      gd->relocaddr);
...
static int reserve_malloc(void)
{
	...
	debug_gan("Reserving %dk for malloc() at: %08lx\n",
			TOTAL_MALLOC_LEN >> 10, gd->start_addr_sp);

#ifdef CONFIG_NONCACHE_MEMORY
	gd->start_addr_sp -= CONFIG_NONCACHE_MEMORY_SIZE;
	debug_gan("Reserving %dk for nocache malloc() at: %08lx\n",
...
static int reserve_board(void)
{
	...
	debug_gan("Reserving %zu Bytes for Board Info at: %08lx\n",
...
static int reserve_global_data(void)
{
	...
	debug_gan("Reserving %zu Bytes for Global Data at: %08lx\n",
...
static int reserve_sysconfig(void)
{
	...
	debug_gan("Reserving %ldk for SYS_CONFIG at: %08lx\n", sys_config_size >> 10,gd->relocaddr);
#endif
	return 0;
}
...
static int reserve_fdt(void)
{
	...
	if (gd->fdt_blob) {
		...
		debug_gan("Reserving %lu Bytes for FDT at: %08lx\n",
...
#  ifdef CONFIG_USE_IRQ
	...
	debug_gan("Reserving %zu Bytes for IRQ stack at: %08lx\n",
...
{
	debug_gan("New Stack Pointer is: %08lx\n", gd->start_addr_sp);
...
static int setup_reloc(void)
{
	...
	debug_gan("Relocating to %08lx, new gd at %08lx, sp at %08lx\n",
