...
void set_working_fdt_addr(void *addr)
{
	...
	working_fdt = buf;
//printf("%s %d---buf=0x%x, addr = %x,working_fdt = %x\n",__FILE__,__LINE__,buf,addr ,working_fdt);	
	setenv_hex("fdtaddr", (ulong)addr);
}
...
printf("%s %d--blob = 0x%x\n",__FILE__,__LINE__,(uint)blob);
