...
{
	sunxi_usb_dbg("buffer = 0x%x,buffer_size =%d\n",(uint)buffer, buffer_size);
	...
	switch(cmd->app_cmd)
	{
		...
		case FEX_CMD_fes_trans:
			...
			//需要发送数据
			{
				...
				sunxi_usb_dbg("usb_efex.c%d len = %d\n", __LINE__, fes_old_data->len);
				if(fes_old_data->len)
				{
					sunxi_usb_dbg("usb_efex.c%d DOU = %d\n", __LINE__, fes_old_data->u2.DOU);
		...
		case FEX_CMD_fes_down:
			sunxi_usb_dbg("FEX_CMD_fes_down\n");
			{
				...
				sunxi_usb_dbg("usb_efex.c%d FEX_CMD_fes_down type = %x\n", __LINE__,trans->type );				
				...
				else	//属于flash数据，分别表示起始扇区，扇区数
				{
					...
					sunxi_usb_dbg("down flash: start 0x%x, sectors 0x%x recv_size = %x\n", trans_data.flash_start, trans_data.flash_sectors,trans_data.recv_size);
				}
				trans_data.last_err          = 0;
				trans_data.app_next_status   = SUNXI_USB_EFEX_APPS_RECEIVE_DATA;
			}
...
//				fes_cmd_verify_status_t *cmd_verify = (fes_cmd_verify_status_t *)cmd_buf;
		...
		case FEX_CMD_fes_tool_mode:
...

			{
				...
				sunxi_usb_dbg("usb_efex.c%d FEX_CMD_fes_down tool_mode = %x\n", __LINE__,fes_work->tool_mode);				
...
printf("usb_efex.c%d dram_data_recv data type=0x%x\n", __LINE__, data_type);
	if(data_type == SUNXI_EFEX_MBR_TAG)			//传输MBR已经完成
        {
		//检查MBR的正确性
...
//sunxi_usb_dbg("usb_efex.c %d sunxi_usb_efex_status = %d\n",__LINE__, sunxi_usb_efex_status);
	...
	switch(sunxi_usb_efex_status)
	{
		case SUNXI_USB_EFEX_IDLE:
			...
			{
				sunxi_usb_dbg("usb_efex.c %d efex_suspend_flag = %d\n",__LINE__, efex_suspend_flag);
		...
		case SUNXI_USB_EFEX_SETUP:		//cbw
			...
			sunxi_usb_dbg("usb_efex.c %d SUNXI_USB_EFEX_SETUP rx_req_length = %d\n", __LINE__,sunxi_ubuf->rx_req_length);
            ...
            {
                sunxi_usb_dbg("usb_efex.c %d----------new cmd format--------\n", __LINE__);
			...
			sunxi_usb_dbg("usb_efex.c%d sunxi_usb_efex_app_step = 0x%x\n",__LINE__, sunxi_usb_efex_app_step);
			if(sunxi_usb_efex_app_step == SUNXI_USB_EFEX_APPS_IDLE)
			{
				sunxi_usb_dbg("usb_efex.c%d APPS: SUNXI_USB_EFEX_APPS_IDLE direction = 0x%x\n",__LINE__, cbw->cmd_package.direction);
				if(cbw->cmd_package.direction == TL_CMD_RECEIVE)	//小机端接收数据
				{
					sunxi_usb_dbg("usb_efex.c%dAPPS: SUNXI_USB_EFEX_APPS_IDLE: TL_CMD_RECEIVE\n", __LINE__);
					sunxi_ubuf->request_size = min(cbw->data_transfer_len, CBW_MAX_CMD_SIZE);
					sunxi_usb_dbg("usb_efex.c%d try to receive data 0x%x, data_transfer_len = %x\n",  __LINE__,sunxi_ubuf->request_size, cbw->data_transfer_len);
					sunxi_usb_efex_write_enable = 0;
					...
					{
						printf("usb_efex.c%d APPS: SUNXI_USB_EFEX_APPS_IDLE: the send data length is 0\n", __LINE__);
				...
				{
					printf("usb_efex.c%d APPS: SUNXI_USB_EFEX_APPS_IDLE: INVALID direction\n", __LINE__);
					printf("usb_efex.c%d sunxi usb efex app cmd err: usb transfer direction is receive only\n",__LINE__);
			...
			{
				sunxi_usb_dbg("usb_efex.c%d APPS: SUNXI_USB_EFEX_APPS_DATA , direction = 0x%x\n", __LINE__,cbw->cmd_package.direction);
				if(cbw->cmd_package.direction == TL_CMD_RECEIVE)	//如果要接收数据，则事先启动dma开始接收
				{
					sunxi_usb_dbg("usb_efex.c%d APPS: SUNXI_USB_EFEX_APPS_DATA: TL_CMD_RECEIVE\n",__LINE__);
					sunxi_ubuf->request_size = MIN(cbw->data_transfer_len, trans_data.recv_size);	//接收长度
					sunxi_usb_dbg("usb_efex.c%d try to receive data 0x%x\n",__LINE__, sunxi_ubuf->request_size);
					sunxi_usb_efex_write_enable = 0;				//设置标志
					if(sunxi_ubuf->request_size)
					{
						sunxi_usb_dbg("usb_efex.c%d dma recv addr = 0x%lx\n",__LINE__, (ulong)trans_data.act_recv_buffer);
						sunxi_udc_start_recv_by_dma(trans_data.act_recv_buffer, sunxi_ubuf->request_size);	//start dma to receive data
					}
					else
					{
						printf("usb_efex.c%d APPS: SUNXI_USB_EFEX_APPS_DATA: the send data length is 0\n",__LINE__);
			...
			{
				sunxi_usb_dbg("usb_efex.c%d APPS: SUNXI_USB_EFEX_APPS_STATUS,direction = 0x%x\n",__LINE__,cbw->cmd_package.direction);
				if(cbw->cmd_package.direction == TL_CMD_TRANSMIT)		//发送数据
				{
					sunxi_usb_dbg("usb_efex.c%d APPS: SUNXI_USB_EFEX_APPS_STATUS: TL_CMD_TRANSMIT\n", __LINE__);
				...
				{
					printf("usb_efex.c%d APPS: SUNXI_USB_EFEX_APPS_STATUS: INVALID direction\n", __LINE__);
			...
			{
				sunxi_usb_dbg("usb_efex.c%d APPS: SUNXI_USB_EFEX_APPS_EXIT\n",__LINE__);
				__sunxi_usb_efex_fill_status();
	  	...
	  	case SUNXI_USB_EFEX_SEND_DATA:
	  		...
	  		sunxi_usb_dbg("usb_efex.c%d  SUNXI_USB_EFEX_SEND_DATA\n", __LINE__);
			{
...
#endif
				sunxi_usb_dbg("usb_efex.c%d send data start 0x%lx, size 0x%x\n",__LINE__, (ulong)trans_data.act_send_buffer, tx_length);
				if(tx_length)
				{
				...
				{
					sunxi_usb_dbg("usb_efex.c%d SUNXI_USB_EFEX_SEND_DATA next: SUNXI_USB_EFEX_APPS_STATUS\n", __LINE__);
					sunxi_usb_efex_app_step = SUNXI_USB_EFEX_APPS_STATUS;
				}
				else if(sunxi_usb_efex_app_step == SUNXI_USB_EFEX_APPS_EXIT)
				{
					sunxi_usb_dbg("usb_efex.c%d SUNXI_USB_EFEX_SEND_DATA next: SUNXI_USB_EFEX_APPS_EXIT\n", __LINE__);
	  	...
	  	case SUNXI_USB_EFEX_RECEIVE_DATA:
	  		...
	  		sunxi_usb_dbg("usb_efex.c%d SUNXI_USB_RECEIVE_DATA sunxi_usb_efex_write_enable=%d\n",__LINE__,sunxi_usb_efex_write_enable);
			if(sunxi_usb_efex_write_enable == 1)		//数据部分接收完毕
			{
				...
				{
					//拷贝到cmd_buf，下次处理也需要
					sunxi_usb_dbg("usb_efex.c%d SUNXI_USB_RECEIVE_DATA: SUNXI_USB_EFEX_APPS_CMD\n",__LINE__);
					if(sunxi_ubuf->request_size != CBW_MAX_CMD_SIZE)		//错误的数据，则返回
					{
						printf("usb_efex.c%d sunxi usb efex err: received cmd size 0x%x is not equal to CBW_MAX_CMD_SIZE 0x%x\n", __LINE__,sunxi_ubuf->request_size, CBW_MAX_CMD_SIZE);
				...
				else if(sunxi_usb_efex_app_step == SUNXI_USB_EFEX_APPS_RECEIVE_DATA)//来自于命令阶段，要求接收数据，下一阶段只能是发送状态(status_t)
				{
					...
					sunxi_usb_dbg("usb_efex.c%d SUNXI_USB_RECEIVE_DATA: SUNXI_USB_EFEX_APPS_RECEIVE_DATA type = 0x%x\n",__LINE__,trans_data.type);
					sunxi_usb_efex_app_step = SUNXI_USB_EFEX_APPS_STATUS;
					if(trans_data.type & SUNXI_EFEX_DRAM_MASK)		//表示属于内存数据，需要事先保存到内存中
					{
						sunxi_usb_dbg("usb_efex.c%d SUNXI_EFEX_DRAM_MASK type\n", __LINE__);
					...
					{
						sunxi_usb_dbg("usb_efex.c%d SUNXI_EFEX_FLASH_MASK\n", __LINE__);
						if(!sunxi_sprite_write(trans_data.flash_start, trans_data.flash_sectors, (void *)trans_data.act_recv_buffer))
						{
							printf("usb_efex.c%d sunxi usb efex err: write flash from 0x%x, sectors 0x%x failed\n",__LINE__, trans_data.flash_start, trans_data.flash_sectors);
							csw.status = -1;
							trans_data.last_err = -1;
						...
						if((uboot_spare_head.boot_data.storage_type == 3)&&(trans_data.type & SUNXI_EFEX_TRANS_FINISH_TAG)&&(fullimg_size == total_write_bytes))
						{
							...
							printf("usb_efex.c%d before sunxi_sprite_setdata_finish\n", __LINE__);
            ...
            {
                sunxi_usb_dbg("usb_efex.c%d SUNXI_USB_EFEX_SETUP_NEW\n", __LINE__);
                   ...
                   ||  FEX_CMD_fes_flash_set_off==  ((struct global_cmd_s *)cmd_buf)->app_cmd )
                {
                    ...
                    {
                        printf("usb_efex.c%d efex queue error: buf_queue_write_all_page fail\n", __LINE__);
                ...
                if(csw.status != 0 )
                {
                    printf("usb_efex.c%d sunxi usb cmd error: 0x%x\n",__LINE__,csw.status);
                    sunxi_ubuf->rx_ready_for_data = 0;
                    sunxi_usb_efex_status = SUNXI_USB_EFEX_IDLE;
                    return -1;
                }
                ...
                sunxi_usb_efex_status_enable = 1;
#endif
                sunxi_usb_dbg("usb_efex.c%d app_next_status = 0x%x\n", __LINE__,trans_data.app_next_status);
                ...
                else if(trans_data.app_next_status == SUNXI_USB_EFEX_APPS_RECEIVE_DATA)
                {
                    ...
                    if(sunxi_ubuf->request_size)
                    {
                        sunxi_usb_dbg("usb_efex.c%d dma recv addr = 0x%lx, size =0x%x\n",__LINE__, (ulong)trans_data.act_recv_buffer,sunxi_ubuf->request_size);
                        sunxi_udc_start_recv_by_dma(trans_data.act_recv_buffer, sunxi_ubuf->request_size);  //start dma to receive data
                    }
                    else
                    {
                        printf("usb_efex.c%d sunxi usb trans error: the request data length is 0\n" , __LINE__);
                ...
                {
                    printf("usb_efex.c%d sunxi usb next status set error:0x%x\n",__LINE__, trans_data.app_next_status);
                    sunxi_ubuf->rx_ready_for_data = 0;
                    sunxi_usb_efex_status = SUNXI_USB_EFEX_IDLE;
            ...
            case SUNXI_USB_EFEX_SEND_DATA_NEW:
                sunxi_usb_dbg("usb_efex.c%d SUNXI_USB_EFEX_SEND_DATA_NEW\n", __LINE__);
                {
                    uint tx_length =  trans_data.send_size;
#if defined(SUNXI_USB_30)
                    sunxi_usb_efex_status_enable = 0;
#endif
                    sunxi_usb_dbg("usb_efex.c%d dma send data start 0x%lx, size 0x%x\n",__LINE__, (ulong)trans_data.act_send_buffer, tx_length);
                    if(tx_length)
                    {
                        sunxi_udc_send_data((void *)trans_data.act_send_buffer, tx_length);
                    }
            ...
            {
                sunxi_usb_dbg("usb_efex.c%d wait dma recv finish...\n",__LINE__);
                ...
                if(!sunxi_usb_efex_write_enable)
                {
                    ...
                    {
                        printf("usb_efex.c%d sunxi efex queue: buf_queue_write_one_page() err\n",__LINE__);
                        efex_write_error_flag = 1;
                    }
#endif
                    break;
                }
                sunxi_usb_dbg("usb_efex.c%d SUNXI_USB_RECEIVE_DATA_NEW type=0x%x\n", __LINE__, trans_data.type);
                ...
                else        //表示当前数据需要写入flash
                {
					...
					sunxi_usb_dbg("usb_efex.c%d flash_start = %x,flash_sectors=%x,act_recv_buffer=%x\n",__LINE__,trans_data.flash_start, trans_data.flash_sectors,(uint)trans_data.act_recv_buffer);
                    if(!sunxi_sprite_write(trans_data.flash_start, trans_data.flash_sectors, (void *)trans_data.act_recv_buffer))
                    {
				 	...
				 	//printf("usb_efex.c%d storage_type=%d, type = 0x%x,fullimg_size= %d, total_write_bytes=%d\n",__LINE__, uboot_spare_head.boot_data.storage_type, trans_data.type,fullimg_size,total_write_bytes);
#ifdef CONFIG_SUNXI_SPINOR
					if((uboot_spare_head.boot_data.storage_type == 3)&&(trans_data.type & SUNXI_EFEX_TRANS_FINISH_TAG)&&(fullimg_size == total_write_bytes))
					{
						//sunxi_usb_dbg("sunxi usb efex trans finish\n");
		...
		case SUNXI_USB_EFEX_STATUS:
			sunxi_usb_dbg("usb_efex.c%d SUNXI_USB_EFEX_STATUS\n", __LINE__);
#if defined(SUNXI_USB_30)
...
#endif
			{
                ...
                sunxi_usb_dbg("usb_efex.c%d efex_write_error_flag = %d\n", __LINE__,efex_write_error_flag);   
                if(efex_write_error_flag)
                {
                        csw.status = -1;
                }
				sunxi_usb_dbg("usb_efex.c%d magic = 0x%x,tag = 0x%x,residue=0x%x,status = 0x%x\n", __LINE__, csw.magic,csw.tag,csw.residue, csw.status);				
				__sunxi_efex_send_status(&csw, sizeof(struct sunxi_efex_csw_t));
				sunxi_usb_dbg("usb_efex.c%d send status end \n", __LINE__ );
			}

			break;

		case SUNXI_USB_EFEX_EXIT:
			sunxi_usb_dbg("usb_efex.c%d SUNXI_USB_EFEX_EXIT efex_write_error_flag = %d\n", __LINE__,efex_write_error_flag);
			...
			if(sunxi_usb_efex_status_enable == 1)
			{
				...
				sunxi_usb_dbg("usb_efex.c%d magic = 0x%x,tag = 0x%x,residue=0x%x,status = 0x%x\n", __LINE__, csw.magic,csw.tag,csw.residue, csw.status);				
				__sunxi_efex_send_status(&csw, sizeof(struct sunxi_efex_csw_t));
				sunxi_usb_dbg("usb_efex.c%d send status end magic\n", __LINE__);
