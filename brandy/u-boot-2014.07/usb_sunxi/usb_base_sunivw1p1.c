...
int sunxi_usb_recv_dma_reconfig(int ram_type)
{
	...
	//配置dma资源
#if 0	
	usb_recv_by_dma.pgsz   = 0;
	usb_recv_by_dma.pgstp  = 0;
	usb_recv_by_dma.cmt_blk_cnt = 0;
#endif	
	...
	usb_recv_by_dma.cfg.dst_drq_type	 = ram_type;//DMAC_CFG_TYPE_DRAM;	//DRAM
...
int sunxi_usb_init(int delaytime)
{
	...
	usb_recv_by_dma.cfg.dst_drq_type     = DMAC_CFG_TYPE_DRAM;  //DRAM
	usb_recv_by_dma.cfg.dst_addr_mode    = DMAC_CFG_DEST_ADDR_TYPE_LINEAR_MODE;
	usb_recv_by_dma.cfg.dst_burst_length = DMAC_CFG_DEST_1_BURST;
	usb_recv_by_dma.cfg.dst_data_width   = DMAC_CFG_DEST_DATA_WIDTH_32BIT;
	...
	usb_send_by_dma.cfg.src_drq_type     = DMAC_CFG_TYPE_DRAM;  //OTG, EP2
	...
	usb_send_by_dma.cfg.dst_drq_type     = DMAC_CFG_DEST_TYPE_OTG_EP1;//DMAC_CFG_DEST_TYPE_OTG_EP1;  //OTG EP1
	...
	usb_send_by_dma.cfg.wait_state       = 4;
	usb_send_by_dma.cfg.continuous_mode  = 0;
...
static int __usb_read_fifo(void *buffer, unsigned int buffer_size)
{
	...
	USBC_Host_FlushFifo(sunxi_udc_source.usbc_hd,USBC_EP_TYPE_RX);
...
int sunxi_udc_start_recv_by_dma(void* mem_base, uint length)
{
	...
	sunxi_usb_dbg("sub_base_sunivw1p1.c%d, length = %x\n",__LINE__, length);
	if(length<=512)
	{
		old_ep_idx = USBC_GetActiveEp(sunxi_udc_source.usbc_hd);
		USBC_SelectActiveEp(sunxi_udc_source.usbc_hd, SUNXI_USB_BULK_OUT_EP_INDEX);			//选择当前EP
