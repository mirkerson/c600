...
//			pending1 = (1 << (dma_channal_source_n[i].channal_count+dma_channal_source_n[i].type));
			/*只处理对应的 full transfer interrupe*/
	...
	for(i=0;i<4;i++)
	{
		...
		{
			pending0 = dma_int->irq_pending;
//			pending1 = (1 << (dma_channal_source_d[i].channal_count+dma_channal_source_d[i].type));
			/*只处理对应的 full transfer interrupe*/
...
//    dma_status->irq_pending = (1 << (dma_channal->channal_count+dma_channal->type));
	//enable dma interrupt
//   	dma_status->irq_en |= (1 << (dma_channal->channal_count+dma_channal->type))
