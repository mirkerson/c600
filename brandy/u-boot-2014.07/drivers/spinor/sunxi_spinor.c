...
#ifdef UBOOT_SPI_TEST

	__u8 *buf_write = 0x80000000;
	__u8 *buf_read  = 0x81000000;
	__u32 id;
	__u32 i;
	
	__spinor_read_id(&id);
	printf("id  = %x\n",id);
	__spinor_erase_block(0);
	__spinor_sector_write(0,32,buf_write);
	__spinor_sector_read(0,32,buf_read);

	for(i=0;i<16*1024;i++)
	{
    	if( *(__u8 *)(0x80000000 + i)!= *(__u8 *)(0x81000000 + i) )
    	{
    		printf("some error has happaned");
    		break;
    	}	    		
	}
...
************************************************************************************************************
*/
int spinor_erase_block(uint block)//suwenrong
{
	__spinor_erase_block(block);

	return 0;
}

int spinor_erase__block0(void)//add by suwenrong
{
    int ret;
    
	ret = __spinor_erase_block(0);
...
int spinor_datafinish(void)
{
	uint  id = 0xff;
	int   ret;
	uint   private_start = 0, private_size = 0;	
	char*  private_buf = NULL;
	char buffer[512] = {0};
	...
	spinor_init_for_boot(0,0);	
	sunxi_partition_init();	
	private_start = sunxi_partition_get_offset_byname("private"); //private partition	
	private_size = sunxi_partition_get_size_byname("private");	
	printf("private_start = 0x%x\n", private_start);	
	printf("private_size =  %u\n", private_size);	
	if(private_start && private_size) {		
		private_buf = (char*)malloc(private_size*512);		
		if(NULL == private_buf)
		{
			printf("malloc fail!\n");
			return -1;
		}
		memset(private_buf, 0xff, private_size*512);				
		sunxi_flash_read(private_start, private_size, private_buf);	
		sunxi_flash_read(private_start + private_size - (8192+512)/512, 1, buffer);
		sunxi_flash_flush();
	} else{	
		printf("fail to get private partition\n");	
	}
	...
	if(private_start && private_size && private_buf) 
	{		
		sunxi_flash_write(private_start, private_size, private_buf);	
		sunxi_flash_write(private_start + private_size - (8192+512)/512, 1, buffer);
		sunxi_flash_flush();	
	}

//	memset(spinor_store_buffer , 0  ,total_write_bytes);
//	__spinor_sector_read(0,total_write_bytes/512 ,spinor_store_buffer);
