...
print_ir_paras:
	...
	if (!IS_ERR_VALUE(nodeoffset)) {
		...
		if (IS_ERR_VALUE(fdt_getprop_u32(working_fdt, nodeoffset, "pmukey_num", &dts_cfg.start_os.pmukey_num))) {
			ARISC_ERR("parse box_start_os->pmukey_num fail\n");
		}
		if (IS_ERR_VALUE(fdt_getprop_u32(working_fdt, nodeoffset, "led_power", &dts_cfg.start_os.led_power))) {
			ARISC_ERR("parse box_start_os->led_power fail\n");
		}
		if (IS_ERR_VALUE(fdt_getprop_u32(working_fdt, nodeoffset, "led_state", &dts_cfg.start_os.led_state))) {
			ARISC_ERR("parse box_start_os->led_state fail\n");
		}
	}
