...
int (* sunxi_flash_erase_pt)(uint block) = sunxi_null_op;//suwenrong
...
int sunxi_flash_erase (uint block)//suwenrong
{
	debug("sunxi flash erase : block %d\n", block);
	return sunxi_flash_erase_pt(block);
}
...
{
	printf("warnning: sunxi spinor not support\n");
	return -1;
}
