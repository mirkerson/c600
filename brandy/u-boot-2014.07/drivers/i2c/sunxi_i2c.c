...
{
	int reg_value = 0;
	reg_value = readl(R_PRCM_BASE + 0xb0);
	reg_value |= 0x01 << 6;
	writel(reg_value,R_PRCM_BASE + 0xb0);
	__msdelay(1);
	reg_value = readl(R_PRCM_BASE + 0x28);
	reg_value |= 0x01 << 6;
	writel(reg_value,R_PRCM_BASE + 0x28);
	__msdelay(1);
...
void i2c_init_cpus(int speed, int slaveaddr)
{
	...
	i2c = (struct sunxi_twi_reg *)SUNXI_CPUS_TWI_BASE;
	set_cpus_i2c_clock();
...
{
	int reg_value = 0;
	reg_value = *((unsigned int *)(R_PRCM_BASE + 0xb0));
	reg_value &= ~(0x01 << 6);
	*((unsigned int *)(R_PRCM_BASE + 0xb0)) = reg_value;
	__msdelay(1);
	reg_value = *((unsigned int *)(R_PRCM_BASE + 0x28));
	reg_value &= ~(0x01 << 6);
	*((unsigned int *)(R_PRCM_BASE + 0x28)) = reg_value;
	__msdelay(1);
	i2c = NULL;
}
