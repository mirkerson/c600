...
static void boot_jump_linux(bootm_headers_t *images, int flag)
{
	...
	r2 = (unsigned long)working_fdt;
	printf("--r2 0x%x,images->ft_len= = %x , working_fdt = %x, bi_boot_params = %x fake = %x\n", (uint)r2, (uint)images->ft_len, (uint)working_fdt,(uint)gd->bd->bi_boot_params, fake);
	if (!fake)
