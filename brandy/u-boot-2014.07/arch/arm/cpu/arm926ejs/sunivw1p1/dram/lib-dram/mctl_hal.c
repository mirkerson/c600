...
//	pll_fator_n = do_div((dram_para->clk*2), 24);
//	pll_fator_n-=1;
//	pll_fator_n = pll_fator_n <<8;
...
//    	reg_val = (0x0<<0) |  (0x0<<4) | pll_fator_n | (0x1u<<31);//N=PLL/12---PLL=12*N(N<=32)---DDR_CLK=6*N--M=2K
    }
    else
    {
    	//M=2,K=2,N=,  enable
    	reg_val = (0x1<<0) |  (0x1<<4) | (((dram_para->clk*2)/24 - 1)<<8) | (0x1u<<31);//N=PLL/24---PLL=24*N(N<=32)---DDR_CLK=12*N--M=K
    	//reg_val = (0x1<<0) |  (0x1<<4) |pll_fator_n | (0x1u<<31);//N=PLL/24---PLL=24*N(N<=32)---DDR_CLK=12*N--M=K
    }


     if(dram_para->cas & (0x1<<4))
     {
     	//调制器模式enable,FREQ = 31.5KHz, FREQ_MODEL:triangular
...
//------------------------------------------------------------------------------------
    ...
    //uart_printf("before dram write delay enble\n");
    if((dram_para->cs_num&0xffff0000)!=0)//add write dqs dq delay
    {
    	//uart_printf("dram write delay enble\n");
