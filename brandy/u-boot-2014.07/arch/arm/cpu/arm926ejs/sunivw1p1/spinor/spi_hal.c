...
//		     asm volatile("mov    r1,  ");
//		   asm volatile("MCR     p15, 0, r8, c7, c14, 1");
//		   asm volatile("mcr p15, 0, %0, c7, c14, 1" : : "r" (i));
	}
}
...
int spic_rw( u32 tcnt, void* txbuf, u32 rcnt, void* rxbuf)
{
	...
	u8 *tx_buffer = txbuf ;
	u8 *rx_buffer = rxbuf;
	writel(0,spi_reg->interrupt_control);
	writel(0xffffffff,spi_reg->interrupt_status);//clear status register

	writel(tcnt, spi_reg->transmit_counter);
	writel(tcnt+rcnt, spi_reg->burst_counter);
	writel(readl(spi_reg->transfer_control)|SPI_EXCHANGE, spi_reg->transfer_control);
	if(tcnt)
	{
		//if(tcnt < 1024)
    	{
			...
			{
				//send data
				while(((readl(spi_reg->fifo_status)>>16)==SPI_FIFO_SIZE) );
				writeb(*(tx_buffer+i),(pspi_reg+SPI_TXD_OFFSET));
				i++;
			}
		}
		//else
	...
	if(rcnt)
	{
		//if(rcnt < 1024)
		{
			...
			{
				//receive valid data
				while(((readl(spi_reg->fifo_status))&0x7f)==0);
				*(rx_buffer+i)=readb(pspi_reg+SPI_RXD);
				i++;
			}
		}
	}

    fcr = readl(spi_reg->fifo_control);
    fcr &= ~(SPI_TXDMAREQ_EN|SPI_RXDMAREQ_EN);
	writel(fcr, spi_reg->fifo_control);
	if ((readl(spi_reg->interrupt_status) & (0xf << 8))|| (timeout==0))	/* (1U << 11) | (1U << 10) | (1U << 9) | (1U << 8)) */
			return RET_FAIL;

	if(readl(spi_reg->transfer_control)&SPI_EXCHANGE)
	{
		printf("XCH Control Error!!\n");
	}

	writel(0xfffff,spi_reg->interrupt_status);  /* clear  flag */
	return RET_OK;
#endif	
}
