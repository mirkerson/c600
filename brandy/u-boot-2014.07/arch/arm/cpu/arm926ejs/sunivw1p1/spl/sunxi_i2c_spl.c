...
void set_i2c_clock(void)
{
    ...
    /* reset i2c clock    */
    /* reset apb2 twi0*/
        reg_value = readl(CCMU_BUS_SOFT_RST_REG2);
        reg_value |= 0x01 << (bus_num +16);
        writel(reg_value, CCMU_BUS_SOFT_RST_REG2);
	__msdelay(1);
#endif
	reg_value = readl(CCMU_BUS_CLK_GATING_REG2);
	reg_value &= ~(1<<(bus_num+16));
	writel(reg_value,CCMU_BUS_CLK_GATING_REG2);
	__msdelay(1);
	reg_value |= (1 <<( bus_num+16));
	writel(reg_value,CCMU_BUS_CLK_GATING_REG2);
...
void i2c_init(int speed, int slaveaddr)
{
	...
	normal_gpio_set_t twi_gpio[2] = {
		{ 8, 0, 2, -1, -1, -1, {0}},//twi0_scl=port:PH0<2><default><default><default>
		{ 8, 1, 2, -1, -1, -1, {0}} //twi0_sda= port:PH1<2><default><default><default>
	};
...
static int axp15X_set_dcdc5(int set_vol)
{
#if 0
	u8  reg_value = 0;
	u8  pmu_type;
	...
	if(set_vol > 0)
	{
		...
		if(axp_i2c_read(AXP15X_ADDR, AXP15X_DDR_VOL_CTRL, &reg_value))
		{
			dbg("sunxi pmu error : unable to read dcdc5\n");
			return -1;
		}
		if(axp_i2c_write(AXP15X_ADDR, AXP15X_DDR_VOL_CTRL, reg_value))
		{
...
int set_ddr_voltage(int set_vol)
{
	...
	return axp15X_set_dcdc5(set_vol);
}
