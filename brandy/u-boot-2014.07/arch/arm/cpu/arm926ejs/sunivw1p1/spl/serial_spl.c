...
#if (1)
	/*boot2.0*/
//	serial_ctrl_base->mcr = 0x3;   //rts & dtr ready
	
	serial_ctrl_base->lcr |= 0x80;	

	bus_apb_clk = get_pll_apb_clk() ;
	
//	uart_clk =do_div(bus_apb_clk + 8 * UART_BAUD , 16 * UART_BAUD) ;
	...
	serial_ctrl_base->ier = 0x00;
	serial_ctrl_base->lcr = 0x80|0x03;
	serial_ctrl_base->dlh = uart_clk>>8;
	serial_ctrl_base->dll = uart_clk&0xff;
	serial_ctrl_base->lcr = 0x03;
	serial_ctrl_base->fcr = 0x01 | 0x02 | 0x04;
#endif
