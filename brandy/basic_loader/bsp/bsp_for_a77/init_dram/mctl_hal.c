...
void auto_set_dram_para(__dram_para_t *para)
{
	...
	switch(ret_val)
	{
		case 2: //ddr2   para->dram_para1-0x10D41000
#ifdef DRAM_TYPE_DDR2
			...
			para->dram_tpr9          = 0x0;
			para->dram_tpr10       	 = 192;
			para->dram_tpr13       	 = 0x27;//default2T:0x07   1T:0x27    Rank, Bank, Row, Column :0x47
#endif
			break;
		case 3: //ddr3
#ifdef DRAM_TYPE_DDR3
			para->dram_zq			= 0xbbbb77;
			para->dram_odt_en       = 1;
			para->dram_para1		= 0x11041000;//COL:11bit  ROW:16bit
			para->dram_para2		= 0x1000;//half dq 1001;default 1000
			para->dram_mr0			= 0x840;
			para->dram_mr1			= 0x0;
			para->dram_mr2			= 0x8;
			...
			para->dram_tpr8          = 0x0;
			para->dram_tpr9          = 0x0;
			para->dram_tpr10       	 = 192;	//min freq
			para->dram_tpr13       	 = 0x3;//default2T:0x07   1T:0x27    Rank, Bank, Row, Column :0x47
#endif
			break;
		case 6:
#ifdef DRAM_TYPE_LPDDR2
			para->dram_zq			= 0xbbbb77;
			para->dram_odt_en       = 1;
			para->dram_para1		= 0x10E41000;//according to design
...
unsigned int mctl_sys_init(__dram_para_t *para)
{
	...
	reg_val &=~(1U<<31);
	writel(reg_val, _CCM_PLL_DDR_REG);
	...
	//ccm_set_pll_ddr_clk(960);//960 for simulation
	//Setup DRAM Clock
	ccm_set_dram_div(1);
...
{
	unsigned int reg_val = 0,ret_val,pad_hold_flag;
...
//	pattern_goto(0x30);
	...
	{
		//bypass all pll  add an external control
//		reg_val=mctl_read_w(PLLGCR);	//Update for aw1681,delete pll_bypass configuration bit,always use pll-bypass
//		reg_val|=(0x1U<<31);
...
//		reg_val=mctl_read_w(DXnGCR0(2));	//16bit for aw1681
//		reg_val&=~(0x1<<1);
...
//		mctl_write_w(reg_val,DXnGCR0(3));
	...
	{
		//pll normal
//		reg_val=mctl_read_w(PLLGCR);	//no pll normal mode for aw1681
//		reg_val&=~(0x1U<<31);
//		mctl_write_w(reg_val,PLLGCR);
		//enable VTC
		reg_val=mctl_read_w(PGCR0);
...
//set DQS Gating mode: default close
	switch(dqs_gating_mode)
	{
		case 0://close DQS gating--auto gating pull down
			...
			break;
		case 1://open DQS gating,default mode
			reg_val=mctl_read_w(PGCR2);
			reg_val&=~(0x3<<6);
			mctl_write_w(reg_val,PGCR2);
		...
		case 2://auto gating pull up
			...
			reg_val =mctl_read_w(DXCCR);//dqs pull up
			reg_val |=(0x1<<27);
			reg_val &=~(0x1U<<31);
...
//setting half DQ
	...
	{
//		mctl_write_w(0,DXnGCR0(2));	//16bit for aw1681,half DQ is 8bit
...
//	pattern_goto(0x80);
	...
	mctl_write_w(reg_val,ZQPR);
	//set zq para   DX2/3,no use for aw1681
//	reg_val = mctl_read_w(ZQPR);
//	reg_val &= ~(0x00ff0000);
...
unsigned int mctl_channel_init(unsigned int ch_index,__dram_para_t *para)
{
	u32 reg_val;
#if 1       //DDR2
	mctl_write_w(0x4299D5,MC_WORK_MODE);//default 0x4298D5--16bit  /0x0x4299D5--32bit
...
//	mctl_write_w(0x0227ffff,MCTL_CTL_BASE+0xc);
	return 1;
}
#endif
...
signed int init_DRAM(int type, __dram_para_t *para)
{
	...
	reg_val = mctl_read_w(MC_CCR);
	reg_val|=(0x1U)<<31;//after initial before write or read must write 1 ~~31bit
	mctl_write_w(reg_val,MC_CCR);
	local_delay(20);
	...
	if(ret_val==0)
		return 0;
//================================================================================
//liuke added for aw1681 DRAM FPGA clear credit twice fail debug 20140515
#ifdef FPGA_PLATFORM
#ifdef SYSTEM_SIMULATION
	for(reg_val = 0;reg_val< 0x100;reg_val++)
		mctl_write_w(DRAM_MEM_BASE+(reg_val<<2),DRAM_MEM_BASE+(reg_val<<2));
	for(reg_val = 0;reg_val< 0x100;reg_val++)
		if(mctl_read_w(DRAM_MEM_BASE+(reg_val<<2)) != DRAM_MEM_BASE+(reg_val<<2))
		{
			pattern_goto(0x12340002);	//add for FPGA simulation
			while(1);
		}
...
//	local_delay(20);
	...
	pattern_goto(0x12340003);	//add for FPGA simulation
	for(reg_val = 0;reg_val< 0x10;reg_val++)
		mctl_write_w((DRAM_MEM_BASE+(reg_val<<2) + 1),DRAM_MEM_BASE+(reg_val<<2));
	for(reg_val = 0;reg_val< 0x10;reg_val++)
		if((mctl_read_w(DRAM_MEM_BASE+(reg_val<<2)))!= DRAM_MEM_BASE+(reg_val<<2) + 1)
		{
			pattern_goto(0x12340004);	//add for FPGA simulation
			while(1);
		}

	while(1);
#endif //end #ifdef  SYSTEM_SIMULATION
#endif //end #ifdef FPGA_PLATFORM
//===========================================================================================================
...
unsigned int mctl_init(void)
{
	...
	//set the parameter
	dram_para.dram_clk			= 792;	//default:480
	...
	//auto_set_test_para(&dram_para);		//added by liuke for aw1681 simulation,20140520
#endif
