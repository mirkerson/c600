...
__s32  BOOT_AnalyzeNandSystem(void)
{
   ...
   //read nand flash chip ID from boot chip
	...
	if( SUPPORT_READ_RETRY && ( (((READ_RETRY_TYPE>>16)&0xff) < 0x10) || (((READ_RETRY_TYPE>>16)&0xff) == 0x40) || (((READ_RETRY_TYPE>>16)&0xff) == 0x41) ))   //boot0 support hynix, micron readretry
	{
		SCAN_DBG("NFC Read Retry Init 0x%x. \n", READ_RETRY_TYPE);
