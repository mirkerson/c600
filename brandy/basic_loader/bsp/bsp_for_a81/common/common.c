...
static void set_pll( void )
{
	__u32 reg_val;
	int delay;
        //ac digital reset ---add by zhanglong
        //reg_val = readl(CCMU_REG_o_APB2_RESET);
        //reg_val |= 0x01;
        //writel(reg_val , CCMU_REG_o_APB2_RESET);
...
void timer_init(void)
{
	...
	writel((0xc << 16) |0x2EE0, TMRC_AVS_DIVISOR); // 0x5db is default by spec
	//init value
...
unsigned int  get_fel_flag(void)
{
    ...
    fel_flag = *(volatile unsigned int *)(0x01c20400 + 0x108);
    return fel_flag;
}
...
{ 
	...
	while(index < 0x18)
	{
		reg_addr = (volatile unsigned int *)(0x01c20400 + 0x100 + index);
		reg_val = *reg_addr;
		msg("reg_addr %x =%x\n", reg_addr, reg_val);
		index+=0x4;
	}
